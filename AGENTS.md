...
**Testing (substantive work):** Follow [conventions.md → Non-negotiables](docs/handbook/conventions.md#non-negotiables-substantive-work-and-agents)—**TDD** (red before production code), **factories** for all domain/API test data ([factories.md](docs/handbook/factories.md)), and **one flat `describe`** per hook spec ([checklist](docs/handbook/conventions.md#hook-and-feature-spec-checklist)). Do not implement hooks first and backfill specs.
