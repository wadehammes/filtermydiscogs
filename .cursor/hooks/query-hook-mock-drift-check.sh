...
if ! run_pnpm --version >/dev/null 2>&1; then
  reason="$(printf 'Query-hook mock drift check: feature test files changed but pnpm is unavailable (install mise tools with `mise install`, then re-run). Manual check: mise exec -- pnpm jest src/tests/utils/queryHookMockRules.spec.ts\n\nChanged test files:\n%s' "$feature_tests_changed")"
...
jest_output=""
jest_status=0
jest_output="$(run_pnpm jest src/tests/utils/queryHookMockRules.spec.ts --runInBand --no-cache 2>&1)" || jest_status=$?
