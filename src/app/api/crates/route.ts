...
import { fetchCrateReleaseCountsByCrateId } from "src/lib/crate-release-counts.server";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const crateIds = crates.map((crate) => crate.id);
    const [previewThumbsByCrateId, releaseCountByCrateId] = await Promise.all([
      fetchCratePreviewThumbs({
        userId: userIdNum,
        crateIds,
      }),
    ...
    const cratesWithCounts = crates.map((crate) => ({
      ...mapCrateRow(crate),
      releaseCount: releaseCountByCrateId.get(crate.id) ?? 0,
