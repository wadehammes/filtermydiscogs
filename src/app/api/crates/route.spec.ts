...
const mockFetchCrateReleaseCountsByCrateId = jest.fn(async () => new Map());

jest.mock("src/lib/crate-release-counts.server", () => ({
  fetchCrateReleaseCountsByCrateId: mockFetchCrateReleaseCountsByCrateId,
}));
...
describe("GET /api/crates", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockFetchCrateReleaseCountsByCrateId.mockResolvedValue(new Map());
  ...
  it("returns paginated crates with release counts", async () => {
    ...
    mockCratesAll.mockResolvedValue([toOrmCrate(crate)]);
    mockFetchCrateReleaseCountsByCrateId.mockResolvedValue(
      new Map([[crate.id, 0]]),
    );
  ...
  it("assigns per-crate release counts from grouped query", async () => {
    const crateA = crateFactory.build({ user_id: USER_ID, id: "a", name: "A" });
    const crateB = crateFactory.build({ user_id: USER_ID, id: "b", name: "B" });
    mockCountRows.mockResolvedValue(2);
    mockCratesAll.mockResolvedValue([toOrmCrate(crateA), toOrmCrate(crateB)]);
    mockFetchCrateReleaseCountsByCrateId.mockResolvedValue(
      new Map([
        ["a", 0],
        ["b", 20],
    ...
    expect(body.data).toEqual([
      expect.objectContaining({ id: "a", releaseCount: 0 }),
      expect.objectContaining({ id: "b", releaseCount: 20 }),
    ]);
  });

  it("orders crates by name ascending", async () => {
    const crate = crateFactory.defaultTestCrate({ user_id: USER_ID });
    mockCountRows.mockResolvedValue(1);
    ...
    expect(dbMock.orm.Crates.orderBy).toHaveBeenCalled();
    expect(dbMock.orm.Crates.limit).toHaveBeenCalledWith(50);
  });
  ...
  it("returns all crates in one page when all=true", async () => {
    ...
    expect(dbMock.orm.Crates.limit).toHaveBeenCalledWith(500);
    await expect(response.json()).resolves.toEqual(
