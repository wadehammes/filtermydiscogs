...
const buildRestoreHarness = (
  ...
  const defaults: Parameters<typeof useRestorePlaybackSessionFromStorage>[0] = {
    isPlaying: false,
...
describe("useReleasePlaybackSessionPersistence", () => {
  ...
  it("usePersistPlaybackSessionOnQueueChange persists when the session queue is present", () => {
  ...
  it("usePersistPlaybackSessionWhilePlaying persists album playback when transport is active and a track position is known", () => {
    const persistPlaybackSession = jest.fn();
    const release = releaseFactory.withDisplayDefaults();
  ...
  it("usePersistPlaybackSessionWhilePlaying skips persist while a pending track position is resolving", () => {
    const persistPlaybackSession = jest.fn();
    const release = releaseFactory.withDisplayDefaults();
  ...
  it("usePersistPlaybackSessionWhilePlaying persists release preview playback without an active album track", () => {
    const persistPlaybackSession = jest.fn();
    const release = releaseFactory.withDisplayDefaults();
  ...
  it("useRestorePlaybackSessionFromStorage marks restore attempted when nothing is persisted", () => {
    const { hasAttemptedRestoreRef, startPlaybackRef } = buildRestoreHarness();

    expect(hasAttemptedRestoreRef.current).toBe(true);
    expect(startPlaybackRef.current).not.toHaveBeenCalled();
  });

  it("useRestorePlaybackSessionFromStorage clears persisted playback for signed-out users", () => {
    writePersistedReleasePlayback({
      instanceId: "1",
      trackPosition: "A1",
    });
  ...
  it("useRestorePlaybackSessionFromStorage restores queue and starts paused playback when the release is in the collection", () => {
    const release = releaseFactory.withDisplayDefaults();
  ...
  it("useRestorePlaybackSessionFromStorage waits for more collection pages before clearing a missing release", () => {
    writePersistedReleasePlayback({
      instanceId: "missing-instance",
      trackPosition: "A1",
    });
