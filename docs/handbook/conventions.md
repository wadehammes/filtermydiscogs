...
- **Shared style modules** under [`src/styles/`](../../src/styles/) (e.g. [`nav-links.module.css`](../../src/styles/nav-links.module.css), [`segmented-control.module.css`](../../src/styles/segmented-control.module.css), [`playback-dock.module.css`](../../src/styles/playback-dock.module.css), [`modal-input.module.css`](../../src/styles/modal-input.module.css), [`modal-dialog.module.css`](../../src/styles/modal-dialog.module.css), [`anchored-popover.module.css`](../../src/styles/anchored-popover.module.css), [`field-label.module.css`](../../src/styles/field-label.module.css), [`scratchpad-textarea.module.css`](../../src/styles/scratchpad-textarea.module.css)) for cross-component patterns. Import directly from the module path; do not re-export through barrel files.
...
- **Mobile filter controls** ([`SearchBar`](../../src/components/SearchBar/SearchBar.module.css), [`Select`](../../src/components/Select/Select.module.css), [`AutocompleteSelect`](../../src/components/AutocompleteSelect/AutocompleteSelect.module.css)): below **`768px`**, align tap targets with **`min-height: var(--touch-target-min)`**, **`1rem`** type, and **`var(--space-3) var(--space-4)`** padding; at **`768px+`** use **`var(--input-height)`** and compact padding. Portaled dropdown **`.option`** rows use **`min-height: 3rem`** and **`var(--space-4)`** padding on mobile for easier tapping ( **`48px`** in [`popoverPlacement.ts`](../../src/utils/popoverPlacement.ts) height estimates). Focus/open states use **`border-color: var(--primary)`** (same as dialog inputs). In **`FiltersDrawer`**, pass **`showLabel`** so each control renders a visible label via [`field-label.module.css`](../../src/styles/field-label.module.css) (uppercase, muted, **`--text-xxs`**).
- **Modal / dialog layout** (native `<dialog>` panels and backdrop dialogs such as **`NoteEditDialog`**, **`EditCrateDialog`**, **`ConfirmDialog`**): compose from [`modal-dialog.module.css`](../../src/styles/modal-dialog.module.css) — **`.shell`** is the shared **`1px`** panel chrome (background, border, radius) plus **`--shadow-modal`** (drop shadow only — no inset ring; **`--shadow-lg`** inset layers stack with the border and read as a double edge). Used by **`.panel`**, **`.backdroppanel`**, and **[`ScrollModal`](../../src/components/shared/ScrollModal/ScrollModal.component.tsx)**. On mobile, top-align panels with **`dvh`** max-height and scrollable backdrops so the keyboard does not clip the title; desktop keeps centered layout at **`width >= 768px`**. Scrollable card modals use **`ScrollModal`** with one outer **`.shell`** border; **`.header`** gets a single **`border-bottom`** divider (not per-row borders on **[`ModalToolbar`](../../src/components/shared/ModalToolbar/ModalToolbar.component.tsx)** or subheaders). Release heroes pass crate/Discogs actions in the toolbar; duplicates modal passes **`title`** plus description text below the toolbar.
...
- **Theming**: Semantic colors live in [`colors.css`](../../src/styles/themes/colors.css) (`:root` defaults including **`--primary: #06c`**) and resolve with **`light-dark()`** from **`color-scheme`**. Hover/active states and shadows use **`color-mix()`**; destructive/success/warning label colors use **`contrast-color()`**. Per-theme primary/link overrides that differ from the default live in [`primary.css`](../../src/styles/themes/primary.css) (**`dark`** and **`light`** use default **`#06c`** from [`colors.css`](../../src/styles/themes/colors.css); **`dim`** uses steel blue `#2d4f6e`; **`slate`**, **`midnight`**, **`futuristic`**, **`high-contrast`** use other tuned blues/cyan). Custom palettes set only **`--background`** (plus pills/gradients) in [`dim.css`](../../src/styles/themes/dim.css) (cool neutral gray), [`sepia.css`](../../src/styles/themes/sepia.css) (warm parchment), [`slate.css`](../../src/styles/themes/slate.css), [`midnight.css`](../../src/styles/themes/midnight.css), [`futuristic.css`](../../src/styles/themes/futuristic.css), and [`high-contrast.css`](../../src/styles/themes/high-contrast.css); shared surface tokens and per-palette **destructive/success/warning** tuning (for contrast on derived **`--card`** surfaces) live in [`palette-derivations.css`](../../src/styles/themes/palette-derivations.css). **`light`** and **`sepia`** inherit the default blue primary. The theme provider sets **`data-theme`** on `<html>` to the resolved palette; **`system`** stores the user preference but applies **`light`** or **`dark`** from the OS. Labels, cycle order, **`themeUsesDarkAssets`**, and **`toSonnerTheme`** (maps any palette to Sonner’s **`light`** / **`dark`**) live in [`themeAppearance.ts`](../../src/utils/themeAppearance.ts).
- **Filled buttons and pills**: Default/hover/active text on solid semantic backgrounds uses **`--primary-foreground`** / **`--on-primary-hover`** / **`--on-primary-active`** (and the matching **`--on-success*`**, **`--on-destructive*`**, **`--on-warning*`** tokens). Set **`background`**, **`border-color`**, and **`color`** together on **`:active`**. Pin **`--on-primary*`** to **`#fff`** for brand blue where **`contrast-color()`** would pick black; use **`contrast-color()`** for other semantic fills and dynamic surfaces (e.g. **`--on-gradient-surface`**, palette-derived **`--foreground`**).
- **Outlined success UI** (`.pillStyle` in [`pills.css`](../../src/styles/pills.css), **`Button`** **`success`** variant, crate selector **New Crate** button): default state uses **`--pill-style-foreground`** / **`--pill-style-border`** (lighter hues in dark mode for WCAG AA on the 15% tint). Hover/active use solid **`--success*`** fills with **`--on-*`** text. **Format pills** use **`--pill-format-*`** the same way with **`--warning*`** on interaction.
...
- **Anchored popovers**: Dropdown panels inside scroll containers (e.g. mobile **`FiltersDrawer`**, mobile **`CrateDrawer`**) use [`anchored-popover.module.css`](../../src/styles/anchored-popover.module.css) and [`useAnchoredPopoverLayout`](../../src/hooks/useAnchoredPopoverLayout.hook.ts). Apply **`popoverAnchor`** + **`anchorStyle`** on the trigger and **`popoverPanel`** (+ **`popoverPanelUpward`** when opening upward) on the menu. Render the panel through [`AnchoredPopoverPortal`](../../src/components/shared/AnchoredPopoverPortal/AnchoredPopoverPortal.component.tsx) (sync **`createPortal`** on the client so layout runs on first open) so **`BottomDrawer`** **`transform`** / **`overflow`** does not clip **`position: fixed`** menus; **`useAnchoredPopoverLayout`** sets **`--popover-fallback-*`** from **`getBoundingClientRect()`** and repositions on **`resize`** plus every scrollable ancestor (scroll does not bubble). Inside **`[data-testid="fmdBottomDrawer"]`**, it also sets **`--popover-z-index: calc(var(--z-5-modal) + 1)`** so portaled menus stack above the drawer shell (default **`--z-6-popover`** is below **`--z-5-modal`**). Up/down placement uses [`popoverPlacement.ts`](../../src/utils/popoverPlacement.ts) (accounts for **`data-bottom-drawer-footer`**, 44px option rows, and caps estimated menu height at **`min(50vh, 300px)`** for placement and max-height).
