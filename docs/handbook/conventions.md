...
- **Inline popover menus** ([`inline-popover-menu.module.css`](../../src/styles/modules/inline-popover-menu.module.css)): **[`UserActions`](../../src/components/StickyHeaderBar/UserActions.tsx)**, **[`FilterViewsMenu`](../../src/components/FilterViewsMenu/FilterViewsMenu.component.tsx)**, **[`ReleaseCrateMenu`](../../src/components/ReleaseCard/ReleaseCrateMenu.component.tsx)**, **[`CrateDetailActionsMenu`](../../src/components/Crates/CrateDetailActionsMenu.component.tsx)**, **[`ThemeSwitcher`](../../src/components/ThemeSwitcher/ThemeSwitcher.component.tsx)** (`variant="menu"`) compose **`list`** / **`item`** / **`footer`** locally in the component module (do not style popups by importing shared class names only in TSX). **`Menu.LinkItem`**: put the composed **`item`** class on **`Menu.LinkItem`**; add a local **`.menuLink`** on the **`Link`** `render` prop (`display: flex`, `color: inherit`, `text-decoration: none`, `width: 100%`) — see **`UserActions.module.css`** / **`FilterViewsMenu.module.css`**. Footer sections use solid **`border-top`** only; **`ReleaseCrateMenu`** alone uses a **dashed** divider between the pinned current crate and the scroll list. **`ReleaseCrateMenu`** below **`1024px`**: rows use **`min-height: var(--touch-target-min)`**, **`--text-base`**, and zero gap between rows; at **`1024px+`** compacts to shared **`.item`** padding.
