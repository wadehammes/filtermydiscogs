...
- **`BackToTop` reveal**: Scroll listener + **`.visible`** class ([`BackToTop.module.css`](../../src/components/BackToTop/BackToTop.module.css)); not scroll-driven animation. On virtualized release views, **`BackToTop`** calls a handler registered via **`usePlaybackPageScrollToTopRegistration`** ([`PlaybackPageShell.context.tsx`](../../src/components/PlaybackPageShell/PlaybackPageShell.context.tsx)) so **`@tanstack/react-virtual`** **`scrollToIndex(0)`** keeps row mounts in sync with scroll position (instant scroll — not smooth **`scrollTo`**).
...
- **Intersection observer mock**: [`.jest/setupTests.ts`](../../.jest/setupTests.ts) installs [`setupIntersectionObserverMock`](../../src/tests/mocks/mockIntersectionObserver.mock.ts) in **`beforeAll`**. Specs that assert observer **`root`**, observed targets, or need to simulate intersection should re-call **`setupIntersectionObserverMock()`** in **`beforeEach`** and use the returned controls — **`triggerIntersection`**, **`getLastObserverRoot`**, **`getObservedElements`**, **`reset`**. The mock defaults **`thresholds`** to **`[0]`** so **`react-intersection-observer`** treats entries as in-view (empty **`thresholds`** breaks configs with **`threshold: 0`**). Prefer this over mocking **`useInView`** directly (e.g. [`useScrollRevealInView`](../../src/hooks/useScrollRevealInView.hook.ts)).
- **Virtualized release grid tests**: [`useVirtualizedReleaseGrid.hook.spec.ts`](../../src/hooks/useVirtualizedReleaseGrid.hook.spec.ts) covers column-count math; [`useReleasesClient.hook.spec.ts`](../../src/hooks/useReleasesClient.hook.spec.ts) asserts the hook exposes the full filtered list (**`visibleReleases`**, no batch windowing); [`ReleasesClient.spec.tsx`](../../src/components/ReleasesClient/ReleasesClient.spec.tsx) **`ReleasesClient virtualization`** describe block asserts bounded card DOM for large collections. Scroll-to-top registration is shared via [`useRegisterPlaybackPageScrollToTop`](../../src/hooks/useRegisterPlaybackPageScrollToTop.hook.ts). **[`BackToTop.po.tsx`](../../src/components/BackToTop/BackToTop.po.tsx)** wraps **`PlaybackPageShellProvider`** with a scroll-element stub for scroll-to-top specs.
