...
- **UI / context / feature-hook tests**: When production code reads data via a query hook, **`jest.mock("src/hooks/queries/use…Query")`** and stub the return value (see [`setupDiscogsReleaseQueryMock`](../../src/tests/mocks/setupDiscogsReleaseQueryMock.ts)). Do **not** mock **`src/api/helpers`** only to drive **`useQuery`** through a real **`QueryClient`**—that couples tests to fetch timing instead of the behavior under test.
- **Real `QueryClient`**: Use **`TestProviders`** / [`createTestQueryClient`](../../src/tests/utils/testQueryClient.tsx) when the component under test is not behind a mocked query hook (e.g. crate list flows). Mock **`src/api/helpers`** for those paths so hooks exercise real query wiring without hitting the network.
