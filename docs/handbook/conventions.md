...
- **Raster images**: Use **`next/image`**. Avoid bare **`<img>`** except rare documented exceptions. Every **`Image`** needs **`alt`**. Discogs covers use **`i.discogs.com`** (allowlisted in `next.config.ts`). Header avatars use **`<img>`** with [`buildDiscogsAvatarProxyUrl`](../../src/utils/discogsAvatarUrl.ts) → **`/api/image-proxy`** (Sharp already resizes; avoids **`next/image`** double optimization and **`localPatterns`** query-string validation).
...
- **No `cloneElement` for styling children.** Do not clone elements to attach **`className`** (e.g. icon hover on SVGs). Wrap children in a styled **`<span>`** or use a shared primitive — **[`IconButton`](../../src/components/IconButton/IconButton.component.tsx)** wraps icons in **`<span className={iconButtonIconClasses(…)} aria-hidden>`** (see [components.md → Icon buttons](components.md#icon-buttons)).
...
| [`modules/`](../../src/styles/modules/) | Shared **CSS Modules** imported by multiple components (**`typography.module.css`**, **`modal-dialog.module.css`**, **`dashboard-card.module.css`**, **`icon-button.module.css`**, **`action-icon-hover.module.css`**, …). Icon hover tokens: see [components.md → Icon buttons](components.md#icon-buttons). |
...
- **Shared style modules** under [`src/styles/`](../../src/styles/) (e.g. [`segmented-control.module.css`](../../src/styles/modules/segmented-control.module.css) — consumed via **[`SegmentedControl`](../../src/components/SegmentedControl/SegmentedControl.component.tsx)** (**`<fieldset>`** + hidden **`<legend>`**); import **`segmentedStyles`** from the component for **`.segment`** on child controls (do not hand-wire **`fieldset`** / **`legend`** in feature TSX); optional **`.segmentCompact`** for denser segments; [`empty-state.module.css`](../../src/styles/modules/empty-state.module.css) via **[`EmptyState`](../../src/components/EmptyState/EmptyState.component.tsx)**; [`text-action.module.css`](../../src/styles/modules/text-action.module.css) for underlined text buttons/links; outer ring via **`.container`** **`border`**, segment dividers via **`border-inline-start`** / **`border-top`** (vertical), [`dashboard-card.module.css`](../../src/styles/modules/dashboard-card.module.css), [`playback-dock.module.css`](../../src/styles/modules/playback-dock.module.css), [`workspace-fab-chrome.module.css`](../../src/styles/modules/workspace-fab-chrome.module.css), [`action-indicator-dot.module.css`](../../src/styles/modules/action-indicator-dot.module.css), [`modal-input.module.css`](../../src/styles/modules/modal-input.module.css), [`modal-dialog.module.css`](../../src/styles/modules/modal-dialog.module.css), [`field-label.module.css`](../../src/styles/modules/field-label.module.css), [`scratchpad-textarea.module.css`](../../src/styles/modules/scratchpad-textarea.module.css), [`menu-item-interaction.module.css`](../../src/styles/modules/menu-item-interaction.module.css), [`inline-popover-menu.module.css`](../../src/styles/modules/inline-popover-menu.module.css). Icon-only control hover: **[`IconButton`](../../src/components/IconButton/IconButton.component.tsx)** / **[`IconButtonLink`](../../src/components/IconButton/IconButtonLink.component.tsx)** (see [components.md → Icon buttons](components.md#icon-buttons)); do not **`composes`** **`icon-button`** in feature CSS or call **`iconButtonClasses()`** in feature TSX; do not **`cloneElement`** to attach icon classes. Import directly from the module path; do not re-export through barrel files. Dashboard bordered panels and release rows compose **[`dashboard-card.module.css`](../../src/styles/modules/dashboard-card.module.css)** — see [patterns.md → Dashboard analytics → Card chrome](patterns.md#dashboard-analytics).
...
- **Mobile filter controls** ([`SearchBar`](../../src/components/SearchBar/SearchBar.module.css), [`Select`](../../src/components/Select/Select.module.css), [`AutocompleteSelect`](../../src/components/AutocompleteSelect/AutocompleteSelect.module.css)): below **`768px`**, align tap targets with **`min-height: var(--touch-target-min)`**, **`1rem`** type, and **`var(--space-3) var(--space-4)`** padding; at **`768px+`** use fixed **`height: var(--input-height)`** (with matching **`min-height`** / **`max-height`**) and compact padding so search, select, and multi-select triggers stay one row tall — including pill **`triggerShell`** rows in **`FiltersBar`**. Portaled dropdown **`.option`** rows use **`min-height: calc(var(--touch-target-min) + var(--space-3))`** and **`var(--space-4)`** padding below **`1024px`** (including mobile **`FiltersDrawer`**); compact rows at **`1024px+`** when desktop **`FiltersBar`** is shown. **[`Select`](../../src/components/Select/Select.component.tsx)** uses **Base UI** [`@base-ui/react/select`](https://base-ui.com/react/components/select) (`Select.Root` / `Trigger` / `Portal` / `Positioner` / `Popup` / `List` / `Item`) with existing CSS Modules; trigger role is **`combobox`** (not **`button`**). **[`AutocompleteSelect`](../../src/components/AutocompleteSelect/AutocompleteSelect.component.tsx)** uses **Base UI** [`@base-ui/react/combobox`](https://base-ui.com/react/components/combobox) with the **input inside popup** pattern (`Combobox.Input` in `Popup` for search). Single-select: full **`Combobox.Trigger`** shows the value + chevron. Multi-select: **`Combobox.Trigger`** with **`nativeButton={false}`** and **`render={<div />}`** wraps the full **`triggerShell`** (pills + chevron) so the pill row is part of the combobox reference for outside-dismiss and single-click open/toggle. Do not nest **`Combobox.Chips`** inside the same trigger — **`Chips`** **`handleInputPress`** opens on **`mousedown`** and races the trigger toggle (double-click to open). Pills use **`pointer-events: none`** on descendants except **`.pillsContainer`** (horizontal chip scroll) and **`.pillClear`**. Placeholder or selection pills live in **`.valueContainer`**; chevron in **`.icon`**. Never nest remove buttons inside a native **`<button>`** trigger. Both filter controls share [`getFilterControlledValue`](../../src/utils/filterControlValue.ts) / [`applyFilterValueChange`](../../src/utils/filterControlValue.ts) and **[`OverlayStack`](../../src/components/OverlayStack/OverlayStack.component.tsx)** / **`usePortaledOverlayContainer`** so **`Select`**, **`AutocompleteSelect`**, and **`FilterViewsMenu`** portal popups into the nearest stack — **[`FiltersBar`](../../src/components/StickyHeaderBar/FiltersBar.tsx)** and **[`BottomDrawer`](../../src/components/BottomDrawer/BottomDrawer.component.tsx)** both set **`escapeStackingContext`** so dropdowns mount on **`document.body`** above **`GlobalPlaybackDock`** (drawer shell **`transform`** / page-shell stacking would otherwise trap in-tree popups); **[`AppDialog`](../../src/components/AppDialog/AppDialog.component.tsx)** keeps in-tree portals. **`FiltersBar`** **`OverlayStack`** passes **`popoverZIndex="calc(var(--z-9-playback-dock) + 1)"`**. Selected pills wrap in **`HorizontalScrollRow`** (**`.pillsContainer`**, **`data-testid="fmdFilterPillsScroll"`**) for horizontal scroll with hidden scrollbars and an end fade (same component as release-card pills; vertical wheel maps to horizontal scroll; **`.pillsContainer`** uses **`touch-action: pan-x`**). **`controlRow`** wraps the trigger whenever **`clearable`** is set (Clear button mounts inside the row only after selections exist; do not wrap/unwrapping the trigger). Pass **`anchor={anchorRef}`** on **`Combobox.Positioner`** with **`anchorRef`** on the multi-select **`Combobox.Trigger`** so **`--anchor-width`** matches the field. **[`FilterViewsMenu`](../../src/components/FilterViewsMenu/FilterViewsMenu.component.tsx)** trigger uses **[`bookmark-thin.svg`](../../src/styles/icons/bookmark-thin.svg)** / **[`bookmark-solid.svg`](../../src/styles/icons/bookmark-solid.svg)** in **`.menuTriggerIcon`** on a primary-tinted control with a **left accent stripe** (filled bookmark when a saved view matches); long view names ellipsize in **`.menuTriggerLabel`** and **`.menuItemText`** (desktop bar root capped at **`13rem`**, menu list **`max-width: min(18rem, …)`**; **`title`** exposes the full name on hover). Both use **`modal={false}`** and **`[data-popup-open]`** / **`[data-highlighted]`** / **`[data-selected]`** on trigger and options — regression tests in **`FiltersDrawer.spec.tsx`**, **`StickyHeaderBar.spec.tsx`**, and **`OverlayStack.spec.tsx`** assert popups mount under **`[data-overlay-stack-portal]`** (drawer in-tree; desktop **`FiltersBar`** escaped on **`document.body`** via **`[data-overlay-stack-escape]`**). In **`FiltersDrawer`**, facet filters use **`facetFilterGroup`** / **`facetFilterControls`** ([`FiltersDrawer.module.css`](../../src/components/FiltersDrawer/FiltersDrawer.module.css)): one shared **`facetLabel`** per facet, then **`AutocompleteSelect`**, **`FilterMatchOperatorSelect`**, and an external **Clear** button inline on one row (Match before Clear; Clear only when selections exist). **`FiltersBar`** **`styleFilterGroup`** column stretch applies only to **`[data-testid="fmdAutocompleteSelect"]`** / **`[data-testid="fmdSelect"]`** wrappers (not every direct child). **[`FiltersBarSkeleton`](../../src/components/StickyHeaderBar/FiltersBarSkeleton.component.tsx)** reuses [`FiltersBar.module.css`](../../src/components/StickyHeaderBar/FiltersBar.module.css) — loading placeholders must mirror **`FiltersBar`** DOM order (**Views** shell with the same bookmark icon and accent stripe → search → three **`styleFilterGroup`** genre/format/year+Match wrappers → sort): each group uses **wrapper test id** → inner **`[data-filter-control-trigger]`** shell in [`FiltersBarSkeleton.module.css`](../../src/components/StickyHeaderBar/FiltersBarSkeleton.module.css)).
- **Modal / dialog layout** (controlled overlays such as **`ConfirmDialog`**, **`ScrollModal`**, **`NoteEditDialog`**, **`EditCrateDialog`**): portaled dialogs use **[Base UI](https://base-ui.com/react/overview/quick-start)** via **[`AppDialog`](../../src/components/AppDialog/AppDialog.component.tsx)** — **`Dialog.Root`** with **`modal="trap-focus"`** (focus trap only; no Base UI document scroll lock) + **`Dialog.Portal`** / **`Dialog.Backdrop`** / **`Dialog.Popup`**, styled with [`base-ui-portal.module.css`](../../src/styles/modules/base-ui-portal.module.css) and [`modal-dialog.module.css`](../../src/styles/modules/modal-dialog.module.css). Compact form/confirm dialogs use **[`FormDialog`](../../src/components/FormDialog/FormDialog.component.tsx)** on top of **`AppDialog`**: shared **`.panel`** chrome (**`backdroppanel`**, **`25rem`** default / **`28rem`** via **`panelWidth="md"`**), inner **`.content`** flex column (**`gap: var(--space-6)`** between header, body, footer — required because **`AppDialog`** wraps children in **`OverlayStack`**, so panel-level **`gap`** does not reach sections), **`.header`** groups **`Dialog.Title`** + optional **`Dialog.Description`** (**`gap: var(--space-2)`**), **`.body`** for fields (**`gap: var(--space-4)`**), **`.footer`** for actions. Use **`FormDialog.Field`**, **`FormDialog.CheckboxField`**, and exported **`formDialogStyles.label`**; do not recompose title/description/panel spacing in feature modules. Consumers: **`CreateCrateDialog`**, **`SaveFilterViewDialog`**, **`ConfirmDialog`**, **`EditCrateDialog`**. **`AppDialog`** and **[`BottomDrawer`](../../src/components/BottomDrawer/BottomDrawer.component.tsx)** call **`usePlaybackPageScrollLock`** from **[`PlaybackPageShell.context.tsx`](../../src/components/PlaybackPageShell/PlaybackPageShell.context.tsx)** so the playback-page scroll root is locked while open on **`PlaybackPageShell`** pages (no-op elsewhere). **`.shell`** is the shared **`1px`** panel chrome (background, border, radius) plus **`--shadow-modal`**. **`ScrollModal`** merges **`styles.modal`** into **`panelClassName`** on the popup — width via **`min(calc(100vw - var(--scroll-modal-horizontal-inset, var(--space-8))), var(--scroll-modal-max-width, 42rem))`** (set **`--scroll-modal-max-width`** on extra classes like **`ReleaseModal`** **`modalWide`** (**`68rem`**) only, not on **`.modal`** itself). Popups cap at **`90svh`** on mobile and **`90dvh`** at **`768px+`** ([`ScrollModal.module.css`](../../src/components/ScrollModal/ScrollModal.module.css), [`modal-dialog.module.css`](../../src/styles/modules/modal-dialog.module.css)). When **`AppDialog`** portals into **`[data-playback-overlay-portal]`**, **`popupModalShell`** applies the same width/max-width **`min(calc(100% - …), var(--scroll-modal-max-width))`** on the popup (**`100%`** of the workspace portal, not **`100vw`** — do not use **`width: auto`**) and height caps with **`calc(100% - var(--space-8))`** so centered modals keep a buffer from filter bars and the in-flow dock (mobile **`min(90svh, calc(100% - var(--space-8)))`**, desktop **`min(90dvh, calc(100% - var(--space-8)))`**). Compose **`AppDialog`** with **`Dialog.Title`** / **`Dialog.Description`** for labelled content; pass **`backdropVariant="modal"`** for crate/note editors and release modals. Tests: popup **`data-testid`** is the dialog root; backdrop dismiss uses **`${testId}-backdrop`**. Scrollable card modals use **`ScrollModal`** with one outer **`.shell`** border; **`.header`** gets a single **`border-bottom`** divider (not per-row borders on **[`ModalToolbar`](../../src/components/ModalToolbar/ModalToolbar.component.tsx)** or subheaders). Release heroes pass crate/Discogs actions in the toolbar; duplicates modal passes **`title`** plus description text below the toolbar. Filter dropdowns (**`Select`**, **`AutocompleteSelect`**) and inline menus inside overlays use Base UI **`Portal`** with **`container`** from **`usePortaledOverlayContainer`** (see **`OverlayStack`**). Header/crate action menus use Base UI **`Menu`** (`Menu.Root` / `Trigger` / `Portal` / `Positioner` / `Popup` / `Item` / `LinkItem` / `Separator`) with **`modal={false}`** and **`[data-highlighted]`** on items — see **Inline popover menus** below.
- **Inline popover menus**: **[`InlinePopoverMenu`](../../src/components/InlinePopoverMenu/InlinePopoverMenu.component.tsx)** wraps Base UI **`Menu.Portal`** / **`Positioner`** / **`Popup`** with shared chrome from [`inline-popover-menu.module.css`](../../src/styles/modules/inline-popover-menu.module.css) — use **`InlinePopoverMenu.Panel`** ( **`positionMethod="fixed"`**, optional **`scrollable`** — **`scrollable`** uses **`.popupScroll`**, which must **`composes`** **`popup`** from [`inline-popover-menu.module.css`](../../src/styles/modules/inline-popover-menu.module.css), not a local re-export), **`useOverlayStack`** for **`OverlayStack`** portal + z-index) plus **`List`** / **`Footer`** / **`Item`** / **`ItemDanger`** / **`ItemNeutral`** / **`Link`** helpers. Shared **`.item`** rows below **`1024px`**: **`min-height: var(--touch-target-min)`**, **`--text-base`**, **`var(--space-3) var(--space-4)`** padding, zero **`.list`** / **`.footer`** / **`.itemGroup`** gap; compact at **`1024px+`**. In **[`InlinePopoverMenu.module.css`](../../src/components/InlinePopoverMenu/InlinePopoverMenu.module.css)**, **`.itemDanger`** / **`.itemNeutral`** must **`composes`** **`item`** from [`inline-popover-menu.module.css`](../../src/styles/modules/inline-popover-menu.module.css) plus the matching hover helper from [`menu-item-interaction.module.css`](../../src/styles/modules/menu-item-interaction.module.css) — do not re-export **`itemdanger`** / **`itemneutral`** (transitive **`composes`** drops base row padding/hover). Do **not** compose **`popup`** / **`list`** / **`item`** directly in feature modules; add only feature-specific **`popupClassName`** / row overrides. Consumers: **[`UserActions`](../../src/components/StickyHeaderBar/UserActions.tsx)**, **[`FilterViewsMenu`](../../src/components/FilterViewsMenu/FilterViewsMenu.component.tsx)**, **[`ReleaseCrateMenu`](../../src/components/ReleaseCard/ReleaseCrateMenu.component.tsx)**, **[`CrateDetailActionsMenu`](../../src/components/Crates/CrateDetailActionsMenu.component.tsx)**, **[`ThemeSwitcher`](../../src/components/ThemeSwitcher/ThemeSwitcher.component.tsx)** (`variant="menu"`). **`Menu.LinkItem`**: **`className={inlinePopoverMenuStyles.item}`** on **`Menu.LinkItem`**, **`inlinePopoverMenuStyles.link`** / **`linkRow`** on the **`Link`** **`render`** prop. Footer sections use solid **`border-top`** only; **`ReleaseCrateMenu`** alone uses a **dashed** divider between the pinned current crate and the scroll list. **`ReleaseCrateMenu`** may still tune row padding via **`--crate-menu-item-pad-inline`**; do not reimplement mobile **`min-height`** in feature modules unless a row needs a different density.
...
- **Theme files** ([`src/styles/themes/`](../../src/styles/themes/)):
  - **`base.css`** — spacing, typography, transitions, z-index, pattern tokens (`--pattern-microdot`, `--pattern-microdot-size`; **`--pattern-microdot-mix`** in **`colors.css`**), component sizing (`--input-height`, **`--touch-target-min`** **`3rem`** below **`768px`**, **`2.75rem`** at **`768px+`** for mobile tap targets).
...
- **Shared elevation / micro-theming**: Prefer global utilities from [`global/theming.css`](../../src/styles/global/theming.css) over copy-pasted shadow stacks. **Do not use inset `box-shadow`** for borders or dividers — use **`border`**, **`outline`**, or segment **`border-inline-start`** / **`border-top`** instead. **Exception:** [`ReleaseCard.module.css`](../../src/components/ReleaseCard/ReleaseCard.module.css) and [`MobileReleaseCard.module.css`](../../src/components/ReleaseCard/MobileReleaseCard.module.css) use inset strokes on the card shell so cover art stays flush with rounded corners. In a CSS Module:
...
- **Layout container queries**: Prefer **`container-type: inline-size`** on component roots when columns should respond to **parent** width — e.g. [`StatsGrid`](../../src/components/StatsGrid/StatsGrid.module.css) (named **`stats-grid`** on **`.container`**, grid on **`.statsGrid`** so **`@container`** rules target a descendant), [`ReleaseCard`](../../src/components/ReleaseCard/ReleaseCard.module.css) (`release-card` → title/meta in [`ReleaseCardTitle.module.css`](../../src/components/ReleaseCard/ReleaseCardTitle.module.css)), [`DashboardHero`](../../src/components/Dashboard/DashboardHero.module.css) **`.metricsList`**, [`FiltersBar`](../../src/components/StickyHeaderBar/FiltersBar.module.css) **`.styleFilterGroup`**. Do not put **`container-type`** and the **`@container { .sameClass }`** rule on the same element — queries only match **descendants**. Style queries remain in [`theming.css`](../../src/styles/global/theming.css).
...
- **Segmented controls**: Use **[`SegmentedControl`](../../src/components/SegmentedControl/SegmentedControl.component.tsx)** — default [`.container`](../../src/styles/modules/segmented-control.module.css) clips segment corners via **`overflow: hidden`** (no per-segment **`border-radius`**); pass **`allowOverflow`** for tooltip overflow ([`ReleaseCardOverlayActions`](../../src/components/ReleaseCard/ReleaseCardOverlayActions.component.tsx)). Desktop overlay **`.segmentSlot .tooltip`** labels in [`ReleaseCard.module.css`](../../src/components/ReleaseCard/ReleaseCard.module.css) show on **`:hover` only** (not **`:focus-within`**) and hide while a portaled menu is open (**`:has([data-popup-open])`**) so labels do not stick when the trigger keeps focus after opening **`ReleaseCrateMenu`**.
...
- **`BackToTop` reveal**: Scroll listener + **`.visible`** class ([`BackToTop.module.css`](../../src/components/BackToTop/BackToTop.module.css)); not scroll-driven animation. On virtualized **list** view ([`ReleasesTable`](../../src/components/ReleasesTable/ReleasesTable.component.tsx)), **`BackToTop`** uses a handler registered via **`useRegisterPlaybackPageScrollToTop`** so **`scrollToIndex(0)`** stays in sync; card view falls back to native scroll on the playback scroll root.- **Subgrid stat alignment**: [`StatsGrid`](../../src/components/StatsGrid/StatsGrid.module.css) + [`StatCard`](../../src/components/StatCard/StatCard.module.css) and [`DashboardHero`](../../src/components/Dashboard/DashboardHero.module.css) **`.metricsList`** use **`grid-template-rows: subgrid`** behind **`@supports`**; **`StatCard`** always renders a third **`.statSubtext`** row (**`:empty`** hides it).
...
Mono text uses **`--text-meta-*`** size tokens (2px smaller than the matching **`--text-*`** scale) because JetBrains Mono runs large at equal pixel sizes. Example: **`--text-meta-sm`** for release-card artist/label lines and modal hero artist; **`--text-meta-xs`** on mobile release rows; **`--text-meta-xxs`** for uppercase field-label eyebrows (e.g. crate menu headings, form labels).
...
- **Shared classes**: [`src/styles/modules/typography.module.css`](../../src/styles/modules/typography.module.css) — `siteWordmark`, `brandEyebrow`, `sectionEyebrow`, `displayHeading`, `lead`, `sectionHeading`, `subsectionHeading`, `bodyText`, `metaCaption`. **`siteWordmark`** — small-caps header/nav label (`font-variant-caps: all-small-caps`, `--letter-spacing-eyebrow`); pair with **`SITE_WORDMARK`** from [`siteMetadata.ts`](../../src/constants/siteMetadata.ts) in logged-out [`PublicPageHeader`](../../src/components/PublicPageHeader/PublicPageHeader.component.tsx) (**`fmd-icon.svg`** + wordmark text; link **`aria-label`** stays **`SITE_NAME`**). Authenticated [`HeaderTitle`](../../src/components/StickyHeaderBar/HeaderTitle.tsx) shows the icon only (**`aria-label`** **`SITE_NAME`**). Compose with local layout classes (width, alignment) in component modules. Screen-reader-only text uses [`src/styles/modules/accessibility.module.css`](../../src/styles/modules/accessibility.module.css) (`.visuallyHidden`).
- **Form field labels** (`<label htmlFor=…>`, filter control labels, dialog field names): compose **[`.label`](../../src/styles/modules/field-label.module.css)** — uppercase muted mono eyebrow (`--font-family-meta`, `--text-meta-xxs`, `--letter-spacing-eyebrow`). Same chrome as **[`ReleaseCrateMenu`](../../src/components/ReleaseCard/ReleaseCrateMenu.module.css)** **`.menuHeading`** (layout padding stays local). Checkbox / toggle copy and button text are not field labels—keep body styles. **`composes`** for field labels must live on a **top-level** class (Turbopack rejects nested selectors like `.dialog .label { composes: … }`).
...
- **Infinite scroll / intersection observer**: [`.jest/setupTests.ts`](../../.jest/setupTests.ts) installs [`setupIntersectionObserverMock`](../../src/tests/mocks/mockIntersectionObserver.mock.ts) in **`beforeAll`**. Specs that assert observer **`root`**, observed targets, or need to simulate sentinel intersection should re-call **`setupIntersectionObserverMock()`** in **`beforeEach`** and use the returned controls — **`triggerIntersection`**, **`getLastObserverRoot`**, **`getObservedElements`**, **`reset`**. The mock defaults **`thresholds`** to **`[0]`** so **`react-intersection-observer`** treats entries as in-view (empty **`thresholds`** breaks configs with **`threshold: 0`**). Prefer this over mocking **`useInView`** directly. Regression examples: [`useReleasesClient.hook.spec.ts`](../../src/hooks/useReleasesClient.hook.spec.ts) (hook + scroll-container wiring), [`ReleasesClient.spec.tsx`](../../src/components/ReleasesClient/ReleasesClient.spec.tsx) (integration). List-view scroll-to-top registration uses [`useRegisterPlaybackPageScrollToTop`](../../src/hooks/useRegisterPlaybackPageScrollToTop.hook.ts); **[`BackToTop.po.tsx`](../../src/components/BackToTop/BackToTop.po.tsx)** wraps **`PlaybackPageShellProvider`** with a scroll-element stub.
...
- **Playback specs**: Mock [`postYoutubePlayerCommand.ts`](../../src/utils/postYoutubePlayerCommand.ts) exports explicitly (**`postYoutubePlayerCommand`**, **`loadAndPlayYoutubeVideo`**, **`loadYoutubeVideoById`**, **`transitionYoutubeIframeToVideo`**, **`requestYoutubePlayerState`**) — see [`releasePlayback.context.spec.tsx`](../../src/context/releasePlayback.context.spec.tsx), [`ReleaseMiniPlayer.spec.tsx`](../../src/components/ReleaseMiniPlayer/ReleaseMiniPlayer.spec.tsx), [`PersistentYoutubeIframe.spec.tsx`](../../src/components/PersistentYoutubeIframe/PersistentYoutubeIframe.spec.tsx). Provider tests simulate YouTube **`message`** events (**`onStateChange`**, **`infoDelivery`**) via helpers such as **`dispatchYoutubePlayerState`** and override **`document.visibilityState`** with **`Object.defineProperty`** for background-tab queue advance. For **`requestYoutubePlayerState`** assertions, prefer **`mock.calls[0]?.[0]`** over **`toHaveBeenCalledWith(iframe)`** (avoids TS2589 deep instantiation on mocked iframe args). **`PersistentYoutubeIframe.spec.tsx`** wraps with **`TestProviders`** + **`ReleasePlaybackProvider`** (not bare **`ReleasePlaybackTestTree`** — needs auth for the provider).
...
- Seed or manipulate the cache in tests: **`queryClient.setQueryData`**, **`invalidateQueries`**, **`queryClient.query`**, **`resetQueries`**, or passing a custom **`queryClient`** solely to control query results (exceptions below).
...
**No `jest.requireActual` in test mocks:** Do not spread **`jest.requireActual(...)`** to pull real implementations into a **`jest.mock`** factory. List each mocked export explicitly (**`jest.fn()`** or a small inline helper). When a component only needs legacy **`useCrate()`** stubs, mock **`useCrate`** alone and render with **`includeCrate: false`**. When it mounts **`ReleaseCrateMenu`**, use the real **`CrateProvider`** with mocked **`api.*`** crate routes (see [`ReleaseListItem.po.tsx`](../../src/components/ReleaseListItem/ReleaseListItem.po.tsx)) or mock **`useCrateState`** + **`useCrateActions`** explicitly with **`includeCrate: false`**. When a route test needs pure helpers from a module that also imports DB code (e.g. **`getPaginationParams`** in [`src/lib/api-helpers.ts`](../../src/lib/api-helpers.ts)), copy the small helper body into the mock factory instead of importing the real module at the top level. When a server module under test imports **`{ Prisma } from "@prisma/client"`** for raw SQL (**`Prisma.sql`** / **`Prisma.join`**, e.g. [`product-analytics-maintenance.server.ts`](../../src/lib/product-analytics-maintenance.server.ts)), mock **`src/lib/db`** as usual and also **`jest.mock("@prisma/client")`** with stub **`Prisma.sql`** / **`Prisma.join`** helpers (see [`product-analytics-maintenance.server.spec.ts`](../../src/lib/product-analytics-maintenance.server.spec.ts))—otherwise Jest must resolve the generated client, which can break after pnpm dependency-hash changes until **`prisma generate`** runs. When only one side-effect export needs stubbing (e.g. **`postYoutubePlayerCommand`**), keep it in a dedicated module ([`postYoutubePlayerCommand.ts`](../../src/utils/postYoutubePlayerCommand.ts)) and **`jest.mock`** that file only—leave pure helpers on the parent module running for real.
...
- **UI / context / feature-hook tests**: Mock **`src/api/urls`** (see **Do not test React Query** above) and render with **`TestProviders`** / **`renderFeatureHook`**. **Context specs** for a single provider (e.g. [`crate.context.spec.tsx`](../../src/context/crate.context.spec.tsx), [`auth.context.spec.tsx`](../../src/context/auth.context.spec.tsx)) should use a **minimal tree** — fresh [`createTestQueryClient`](../../src/tests/utils/testQueryClient.tsx) per test plus only the providers the subject needs (**`QueryClientProvider`** → **`AuthProvider`** → **`CrateProvider`**), not full **`TestProviders`** (avoids mounting collection sync, filters, theme, and analytics for every case). When **`CrateProvider`** mounts, stub crate endpoints with [`setupDefaultCrateApiMocks`](../../src/tests/mocks/setupDefaultCrateApiMocks.ts). Crate drawer component specs use [`setupCrateDrawerTests`](../../src/components/CrateDrawer/crateDrawerTestSetup.ts), which stubs both **`api.crates`** and **`api.crate`** for the default **`crate-1`** fixture—override **`api.crate`** in individual tests when release data matters. Use **`waitFor`** when asserting async outcomes after **`api.*`** resolves.
...
- **Do mock**: External dependencies—**`src/api/urls`** via the **canonical mock pattern** above; configure responses with [`mockApiResponse`](../../src/tests/mocks/mockApiResponse.ts). Use [`mockFetchResponse`](../../src/tests/mocks/mockFetchResponse.ts) for fetch-level tests of [`src/api/endpoints/`](../../src/api/endpoints/). Also mock auth cookies/services, `next/navigation`, **`IntersectionObserver`** (see **Infinite scroll / intersection observer** above), and similar.
- **Authenticated tests + `CrateProvider`**: Default **`TestProviders`** includes **`CrateProvider`**, which runs **`useCratesQuery`** / **`useCrateQuery`** when **`authInitialState`** is authenticated and **`isCheckingAuth`** is false. If the test mocks **`src/api/urls`** but only stubs unrelated endpoints (e.g. collection fields), React Query will still call **`api.crates`** / **`api.crate`**—undefined mocks log **`Query data cannot be undefined`**. Call [`setupDefaultCrateApiMocks`](../../src/tests/mocks/setupDefaultCrateApiMocks.ts) in the PO **`setupMocks()`** or hook test **`beforeEach`** when default empty crate data is enough (also stubs **`api.releaseCrateMembership`** with empty **`crateIds`**, and **`api.discogsRelease`** / **`api.discogsReleaseBatch`** via [`setupFetchDiscogsReleaseMock`](../../src/tests/mocks/setupFetchDiscogsReleaseMock.ts) so **`useReleaseCrateMembershipQuery`**, **`useDiscogsReleaseQuery`**, and playback/modal prefetch do not hit undefined); override **`api.crate`** (see [`CrateSelector.po.tsx`](../../src/components/CrateSelector/CrateSelector.po.tsx)) when the test needs specific crate IDs or release counts—not by seeding **`QueryClient`** cache. Prefer **`jest.clearAllMocks()`** over **`jest.resetAllMocks()`** when re-applying **`setupDefaultCrateApiMocks`** / **`setupFetchDiscogsReleaseMock`** in the same **`beforeEach`**—**`resetAllMocks`** clears mock implementations globally for the shared **`api`** module and can break the next spec's release-detail stubs.
...
- **`window.location` / `window.history`**: [`.jest/setupTests.ts`](../../.jest/setupTests.ts) installs [**`jest-location-mock`**](https://www.npmjs.com/package/jest-location-mock) via **`replaceLocation()`** / **`replaceHistory()`** in **`beforeAll`** and **`resetWindowLocation()`** each test. Set the URL in specs with [`setWindowLocation`](../../src/tests/mocks/mockWindowLocation.ts) (assigns **`window.location.href`**) instead of ad hoc **`Object.defineProperty(window.location, …)`** spies.
...
- **Base UI (jsdom)**: [`.jest/setupTests.ts`](../../.jest/setupTests.ts) also polyfills **`PointerEvent`**, mocks **`HTMLElement.prototype.getBoundingClientRect`**, and injects CSS so portaled popups are interactable in tests (`[data-starting-style]` / `[data-ending-style]`). Specs for **`Select`**, **`AutocompleteSelect`**, **`UserActions`**, and **`CrateDetailActionsMenu`** use **`userEvent.setup({ pointerEventsCheck: 0 })`** so jsdom can open Base UI popups; query portaled **`option`** / **`menuitem`** nodes with **`{ hidden: true }`** when needed. Shared filter-control helpers in [`filterControlTestHelpers.ts`](../../src/tests/filterControlTestHelpers.ts): **`openFilterCombobox`** (click the trigger chevron — not pill text — **`waitFor`** a fresh **`aria-expanded="true"`** on the combobox, then wait for the search placeholder), **`openFilterSelect`**, **`selectSingleFilterOption`** (single **`Select`** — click option, wait for close, **`Escape`** only if still open), **`selectMultiFilterOption`** (**`AutocompleteSelect`** / multi **`Select`** — click option, then **`closeOpenFilterComboboxes`**), and **`expectFilterComboboxesClosed`**. Do not use one helper for both modes — multi-select keeps the popup open after selection. **`expectFilterPopupAboveBottomDrawer`** and **`expectFilterPopupAbovePlaybackDock`** assert portaled filter popups escape drawer/page stacking — see **`FiltersDrawer.spec.tsx`**, **`StickyHeaderBar.spec.tsx`**, and **`OverlayStack.spec.tsx`**. **`BottomDrawer.spec.tsx`** asserts **`closeButtonPlacement="floating"`** keeps **`floatingShellClose`** as the first child of **`fmdBottomDrawer`** (**`drawerShell`**) outside the header row. Test combobox **filtering and empty-state copy** in **`AutocompleteSelect.spec.tsx`** (isolated); that spec also covers multi-select pill-row open/toggle, scroll via **`fmdFilterPillsScroll`**, and pill remove without opening the popup (**`.pillClear`** must **`stopPropagation`** on **`pointerdown`** / **`mousedown`** — Base UI combobox triggers open on **`mousedown`**, not click). **`FiltersDrawer.spec.tsx`** smoke-tests drawer wiring (open controls + stacking) rather than full combobox behavior. **`Select.spec.tsx`** and **`AutocompleteSelect.spec.tsx`** also assert **reopen on the first click after selecting a value** (including **`clearable`** variants) so trigger/anchor DOM stays stable when pills or a Clear button appear. After opening a combobox or menu manually, **`waitFor`** **`aria-expanded="true"`** on the trigger or for the **`menu`** role to appear.
...
- Sub-surface hooks when the root is not the right query target: e.g. **`fmdFilterPillsScroll`** on multi-select **`AutocompleteSelect`** pill rows (**`HorizontalScrollRow`** inside the combobox trigger), **`fmdReleaseCardPillsRow`** on desktop **`ReleaseCard`** tag rows.
