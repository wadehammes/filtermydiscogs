...
- **Playback specs**: Mock [`postYoutubePlayerCommand.ts`](../../src/utils/postYoutubePlayerCommand.ts) exports explicitly (**`postYoutubePlayerCommand`**, **`loadAndPlayYoutubeVideo`**, **`loadYoutubeVideoById`**, **`transitionYoutubeIframeToVideo`**, **`requestYoutubePlayerState`**) — see [`releasePlayback.context.spec.tsx`](../../src/context/releasePlayback.context.spec.tsx), [`ReleaseMiniPlayer.spec.tsx`](../../src/components/ReleaseMiniPlayer/ReleaseMiniPlayer.spec.tsx), [`PersistentYoutubeIframe.spec.tsx`](../../src/components/PersistentYoutubeIframe/PersistentYoutubeIframe.spec.tsx). Simulate YouTube **`message`** events via **[`dispatchYoutubePlayerState`](../../src/tests/utils/playbackTestHelpers.ts)** (wraps **`act`**) and override **`document.visibilityState`** with **`Object.defineProperty`** for background-tab queue advance. Component playback specs that mount **`CrateProvider`** should **`await settleProviderEffects()`** (see [`settleProviderEffects.ts`](../../src/tests/utils/settleProviderEffects.ts)) and wait for crate queries to resolve before starting playback. Use **[`setupPlaybackSpecTimers`](../../src/tests/utils/playbackTestHelpers.ts)** / **`teardownPlaybackSpecTimers`** in **`beforeEach`**/**`afterEach`**, **`setupPlaybackUser()`** (**`userEvent.setup({ advanceTimers: jest.advanceTimersByTime })`**) for dock interactions, and **`flushPlaybackGestureRetries`** after playback starts or dock controls change — do **not** wrap **`userEvent.click`** in an outer **`act()`**. For **`requestYoutubePlayerState`** assertions, prefer **`mock.calls[0]?.[0]`** over **`toHaveBeenCalledWith(iframe)`** (avoids TS2589 deep instantiation on mocked iframe args). **`PersistentYoutubeIframe.spec.tsx`** wraps with **`TestProviders`** + **`ReleasePlaybackProvider`** (not bare **`ReleasePlaybackTestTree`** — needs auth for the provider). Do **not** suppress **`act`** warnings in Jest setup — fix async provider/timer setup instead (**`IS_REACT_ACT_ENVIRONMENT`** is set in [`.jest/setEnvVars.ts`](../../.jest/setEnvVars.ts)).
