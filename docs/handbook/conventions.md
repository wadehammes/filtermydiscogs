...
- **Base UI (jsdom)**: [`.jest/setupTests.ts`](../../.jest/setupTests.ts) also polyfills **`PointerEvent`**, mocks **`HTMLElement.prototype.getBoundingClientRect`**, and injects CSS so portaled popups are interactable in tests (`[data-starting-style]` / `[data-ending-style]`). Specs for **`Select`**, **`AutocompleteSelect`**, **`UserActions`**, and **`CrateDetailActionsMenu`** use **`userEvent.setup({ pointerEventsCheck: 0 })`** so jsdom can open Base UI popups; query portaled **`option`** / **`menuitem`** nodes with **`{ hidden: true }`** when needed. Shared filter-control helpers in [`filterControlTestHelpers.ts`](../../src/tests/filterControlTestHelpers.ts): **`openFilterCombobox`** (click the trigger chevron — not pill text — **`waitFor`** a fresh **`aria-expanded="true"`** on the combobox, then wait for the search placeholder), **`openFilterSelect`**, and **`clickFilterOption`** (click the portaled **`option`**, press **`Escape`**, then **`waitFor`** no open comboboxes — multi-select keeps the popup open after selection). **`expectFilterPopupAboveBottomDrawer`** and **`expectFilterPopupAbovePlaybackDock`** assert portaled filter popups escape drawer/page stacking — see **`FiltersDrawer.spec.tsx`**, **`StickyHeaderBar.spec.tsx`**, and **`OverlayStack.spec.tsx`**. **`BottomDrawer.spec.tsx`** asserts **`closeButtonPlacement="floating"`** keeps **`floatingShellClose`** as the first child of **`fmdBottomDrawer`** (**`drawerShell`**) outside the header row. Test combobox **filtering and empty-state copy** in **`AutocompleteSelect.spec.tsx`** (isolated); that spec also covers multi-select pill-row open/toggle, scroll via **`fmdFilterPillsScroll`**, and pill remove without opening the popup. **`FiltersDrawer.spec.tsx`** smoke-tests drawer wiring (open controls + stacking) rather than full combobox behavior. **`Select.spec.tsx`** and **`AutocompleteSelect.spec.tsx`** also assert **reopen on the first click after selecting a value** (including **`clearable`** variants) so trigger/anchor DOM stays stable when pills or a Clear button appear. After opening a combobox or menu manually, **`waitFor`** **`aria-expanded="true"`** on the trigger or for the **`menu`** role to appear.
...
- Sub-surface hooks when the root is not the right query target: e.g. **`fmdFilterPillsScroll`** on multi-select **`AutocompleteSelect`** pill rows (**`HorizontalScrollRow`** inside the combobox trigger).
