...
- **Shared style modules** under [`src/styles/`](../../src/styles/) (e.g. [`nav-links.module.css`](../../src/styles/nav-links.module.css), [`segmented-control.module.css`](../../src/styles/segmented-control.module.css) — **`.container`** on a **`<div>`** (e.g. [`ViewToggle`](../../src/components/ViewToggle/ViewToggle.component.tsx)) or **`<fieldset>`** when the group needs a label; **`<legend>`** uses **`.legend`** plus [`accessibility.module.css`](../../src/styles/accessibility.module.css) **`.visuallyHidden`** in TSX (do not **`composes`** — Stylelint / CSS Modules casing); optional **`.segmentCompact`** for denser segments; outer ring via **`.container::after`** inset shadow so segment hover fills do not cover the border, [`dashboard-card.module.css`](../../src/styles/dashboard-card.module.css), [`playback-dock.module.css`](../../src/styles/playback-dock.module.css), [`modal-input.module.css`](../../src/styles/modal-input.module.css), [`modal-dialog.module.css`](../../src/styles/modal-dialog.module.css), [`field-label.module.css`](../../src/styles/field-label.module.css), [`scratchpad-textarea.module.css`](../../src/styles/scratchpad-textarea.module.css)) for cross-component patterns. Import directly from the module path; do not re-export through barrel files. Dashboard bordered panels and release rows compose **[`dashboard-card.module.css`](../../src/styles/dashboard-card.module.css)** — see [patterns.md → Dashboard analytics → Card chrome](patterns.md#dashboard-analytics).
...
- **Mobile filter controls** ([`SearchBar`](../../src/components/SearchBar/SearchBar.module.css), [`Select`](../../src/components/Select/Select.module.css), [`AutocompleteSelect`](../../src/components/AutocompleteSelect/AutocompleteSelect.module.css)): below **`768px`**, align tap targets with **`min-height: var(--touch-target-min)`**, **`1rem`** type, and **`var(--space-3) var(--space-4)`** padding; at **`768px+`** use fixed **`height: var(--input-height)`** (with matching **`min-height`** / **`max-height`**) and compact padding so search, select, and multi-select triggers stay one row tall — including pill **`triggerShell`** rows in **`FiltersBar`**. Portaled dropdown **`.option`** rows use **`min-height: 3rem`** and **`var(--space-4)`** padding on mobile for easier tapping. **[`Select`](../../src/components/Select/Select.component.tsx)** uses **Base UI** [`@base-ui/react/select`](https://base-ui.com/react/components/select) (`Select.Root` / `Trigger` / `Portal` / `Positioner` / `Popup` / `List` / `Item`) with existing CSS Modules; trigger role is **`combobox`** (not **`button`**). **[`AutocompleteSelect`](../../src/components/AutocompleteSelect/AutocompleteSelect.component.tsx)** uses **Base UI** [`@base-ui/react/combobox`](https://base-ui.com/react/components/combobox) with the **input inside popup** pattern (`Combobox.Input` in `Popup` for search). Single-select: full **`Combobox.Trigger`** shows the value + chevron. Multi-select: always use **`.triggerShell`** — placeholder or selection pills live in **`.valueContainer`**, with a full-field **`.triggerOverlay`** **`Combobox.Trigger`** (absolute inset, chevron aligned right) — **`.pill`** uses **`pointer-events: none`** so clicks open the menu; only **`.pillClear`** remove buttons use **`pointer-events: auto`** above the overlay (never nested inside **`Combobox.Trigger`**). Keep the trigger/anchor DOM stable when the first pill appears or when **`clearable`** shows a **Clear** button: **`controlRow`** wraps the trigger whenever **`clearable`** is set (Clear button mounts inside the row only after selections exist; do not wrap/unwrapping the trigger). Pass **`anchor={anchorRef}`** on **`Combobox.Positioner`** with **`anchorRef`** on **`.triggerShell`** for multi-select (on the full trigger for single-select) so **`--anchor-width`** matches the field. Both filter controls share [`getFilterControlledValue`](../../src/utils/filterControlValue.ts) / [`applyFilterValueChange`](../../src/utils/filterControlValue.ts) and [`useFilterControlPositionerZIndex`](../../src/hooks/useFilterControlPositionerZIndex.hook.ts). Both use **`modal={false}`**, context z-index in **`FiltersBar`** / **`FiltersDrawer`**, and **`[data-popup-open]`** / **`[data-highlighted]`** / **`[data-selected]`** on trigger and options. In **`FiltersDrawer`**, pass **`showLabel`** so each control renders a visible label via [`field-label.module.css`](../../src/styles/field-label.module.css) (uppercase, muted, **`--text-xxs`**). **`FiltersBar`** **`styleFilterGroup`** column stretch applies only to **`[data-testid="fmdAutocompleteSelect"]`** / **`[data-testid="fmdSelect"]`** wrappers (not every direct child). **[`FiltersBarSkeleton`](../../src/components/StickyHeaderBar/components/FiltersBarSkeleton.component.tsx)** reuses [`FiltersBar.module.css`](../../src/components/StickyHeaderBar/components/FiltersBar.module.css) — genre/Match loading placeholders must mirror that DOM (**wrapper test id** → inner **`[data-filter-control-trigger]`** shell in [`FiltersBarSkeleton.module.css`](../../src/components/StickyHeaderBar/components/FiltersBarSkeleton.module.css)).
- **Modal / dialog layout** (controlled overlays such as **`ConfirmDialog`**, **`ScrollModal`**, **`NoteEditDialog`**, **`EditCrateDialog`**): portaled dialogs use **[Base UI](https://base-ui.com/react/overview/quick-start)** via **[`AppDialog`](../../src/components/shared/AppDialog/AppDialog.component.tsx)** — **`Dialog.Root`** with **`modal="trap-focus"`** (focus trap only; no Base UI document scroll lock) + **`Dialog.Portal`** / **`Dialog.Backdrop`** / **`Dialog.Popup`**, styled with [`base-ui-portal.module.css`](../../src/styles/base-ui-portal.module.css) and [`modal-dialog.module.css`](../../src/styles/modal-dialog.module.css). **`AppDialog`** and **[`BottomDrawer`](../../src/components/BottomDrawer/BottomDrawer.component.tsx)** call **`usePlaybackPageScrollLock`** from **[`PlaybackPageShell.context.tsx`](../../src/components/ReleasePlayback/PlaybackPageShell.context.tsx)** so the playback-page scroll root is locked while open on **`PlaybackPageShell`** pages (no-op elsewhere). **`.shell`** is the shared **`1px`** panel chrome (background, border, radius) plus **`--shadow-modal`**. **`AppDialog`** popup classes only handle centering and z-index — **`panelClassName`** owns **`max-width`** (e.g. **`ConfirmDialog`** / **`EditCrateDialog`** modules, **`ScrollModal`** **`--scroll-modal-max-width`** default **`42rem`**). Mobile popups cap at **`90svh`** ([`ScrollModal.module.css`](../../src/components/shared/ScrollModal/ScrollModal.module.css), [`modal-dialog.module.css`](../../src/styles/modal-dialog.module.css)); **`768px+`** uses **`90dvh`**. Compose **`AppDialog`** with **`Dialog.Title`** / **`Dialog.Description`** for labelled content; pass **`backdropVariant="modal"`** for crate/note editors and release modals. Tests: popup **`data-testid`** is the dialog root; backdrop dismiss uses **`${testId}-backdrop`**. Scrollable card modals use **`ScrollModal`** with one outer **`.shell`** border; **`.header`** gets a single **`border-bottom`** divider (not per-row borders on **[`ModalToolbar`](../../src/components/shared/ModalToolbar/ModalToolbar.component.tsx)** or subheaders). Release heroes pass crate/Discogs actions in the toolbar; duplicates modal passes **`title`** plus description text below the toolbar. Filter dropdowns (**`Select`**, **`AutocompleteSelect`**) use Base UI **`Positioner`** / **`Portal`** with context z-index. Header/crate action menus use Base UI **`Menu`** (`Menu.Root` / `Trigger` / `Portal` / `Positioner` / `Popup` / `Item` / `LinkItem` / `Separator`) with **`modal={false}`** and **`[data-highlighted]`** on items.
...
- **Exceptions**: Some dense UI keeps **`--text-xxs`** (not `--text-meta-*`) on mono lines—e.g. catalog pills in [`pills.css`](../../src/styles/pills.css). Prefer meta size tokens for new mono lines unless the design needs the extra-small sans-scale size.
...
- **`TestProviders` collection sync**: Includes **`CollectionDataSync`** by default (aligned with production **`Providers`**). Pass **`includeCollectionSync: false`** when a test seeds collection/filter state manually (**[`SeedCollectionFilters`](../../src/tests/utils/seedCollectionFilters.tsx)**) or asserts pristine **`CollectionContextProvider`** defaults. Component POs such as [`FiltersDrawer.po.tsx`](../../src/components/FiltersDrawer/FiltersDrawer.po.tsx) forward **`sessionFilters`** through **`SeedCollectionFilters`**.
...
- **Tracklist specs (queue + play)**: When embeddable videos exist, each row has a play **`button`** and an **Add to queue** control (**`data-testid="fmdReleaseTrackQueueButton"`**). When **`onTrackSelect`** is omitted (no embeddable videos), rows are static text—assert titles with **`getByText`**, not play **`button`** roles. **`getByRole("button", { name: /track title/ })`** can match both play and queue controls when playback is available—prefer **`getByText`** for the title, **`getAllByTestId("fmdReleaseTrackQueueButton")`**, or row-scoped queries (see [`ReleaseTracklist.spec.tsx`](../../src/components/ReleaseModal/ReleaseTracklist.spec.tsx), [`ReleaseModal.spec.tsx`](../../src/components/ReleaseModal/ReleaseModal.spec.tsx)).
