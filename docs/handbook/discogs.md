...
**Discogs request pacing:** [`discogs-oauth.service.ts`](../../src/services/discogs-oauth.service.ts) serializes outbound Discogs HTTP through [`discogs-request-throttle.ts`](../../src/lib/discogs-request-throttle.ts). The throttle is **reentrancy-safe** (nested calls inside an active slot run immediately via **`AsyncLocalStorage`**) and rejects waiters after **`DISCOGS_THROTTLE_QUEUE_TIMEOUT_MS`** (default **30s**) so a stuck slot cannot pin an invocation for the full Fluid timeout. Prefer a single throttle boundary — do not wrap **`makeAuthenticatedRequest`** / **`makeConsumerRequest`** in another **`runThrottledDiscogsRequest`**. **`getIdentity`** (OAuth callback / session verify) uses **`skipThrottle: true`** so login never waits behind collection pacing. Every Discogs HTTP call goes through [`discogsFetch`](../../src/lib/discogs-fetch.ts) with **`AbortSignal.timeout`** (**`DISCOGS_FETCH_TIMEOUT_MS`**, default **20s**). After each Discogs response, [`discogs-rate-limit.ts`](../../src/lib/discogs-rate-limit.ts) reads **`X-Discogs-Ratelimit`**, **`X-Discogs-Ratelimit-Used`**, and **`X-Discogs-Ratelimit-Remaining`** and adjusts spacing before the next call on that serverless instance (faster when quota remains, slower when the window is nearly exhausted). When Discogs omits those headers, pacing falls back to **`DISCOGS_MIN_REQUEST_INTERVAL_MS`** (default **1000** ms; set **0** to disable the fallback floor).
...
Client helper: **`api.discogsRelease`** in [`src/api/urls.ts`](../../src/api/urls.ts) (forwards React Query **`AbortSignal`** so stale prefetches abort). React Query: **`useDiscogsReleaseQuery`** ([`src/hooks/queries/useDiscogsReleaseQuery.ts`](../../src/hooks/queries/useDiscogsReleaseQuery.ts)) with **`DiscogsReleaseQueryKeys`**. Release-open hover/click prefetches share one global timer and cancel other in-flight **`discogsRelease`** queries before starting a new one ([`releaseOpenPrefetch.ts`](../../src/utils/releaseOpenPrefetch.ts)) so rapid card hovers/clicks do not pile up behind the server Discogs throttle.
