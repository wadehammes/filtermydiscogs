...
| 4 | `GET /api/auth/check` | Verify tokens still valid; return **`avatarUrl`** when profile fetch succeeds |
| 5 | `POST /api/auth/logout` | Clear session cookies; clear cached **`avatarUrl`** for the user |
...
Client reads username via [`getUsernameFromCookies`](../../src/services/auth.service.ts) (`js-cookie`). User ID and **`avatarUrl`** come from [`/api/auth/check`](../../src/app/api/auth/check/route.ts) (verified OAuth identity; avatar via **`GET /users/{username}`** through [`resolveDiscogsAvatarUrl`](../../src/lib/user-profile.server.ts) with a 24h in-memory cache and in-flight request dedup in [`user-profile-cache.ts`](../../src/lib/user-profile-cache.ts)) and are stored in [`AuthProvider`](../../src/context/auth.context.tsx)—not from a client-readable cookie. Header avatars render through [`buildDiscogsAvatarProxyUrl`](../../src/utils/discogsAvatarUrl.ts) → **`/api/image-proxy`**. Access tokens are **never** exposed to client JS.
...
- **`getIdentity`**, **`getUserProfile`** ( **`avatar_url`** for header account pill), **`getCollection`**, **`getCollectionFields`**, **`updateCollectionInstanceField`**, **`api.updateReleaseRating`**, **`deleteReleaseRating`**, **`getCollectionValue`**, **`search`**, release fetches
...
**Read-only Discogs proxies** (`GET /api/collection`, `/api/collection/fields`, `/api/collection/value`, `/api/search`, authenticated `/api/release/[id]`, **`POST /api/release/batch`**) use **`requireReadOnlyDiscogsUser`** / **`getReadOnlyVerifiedUserFromRequest`** so a verified identity cached within the stale window (default 30 minutes) is reused during long collection pagination without re-calling `oauth/identity` on every page. When the in-memory cache is cold (new serverless instance), read-only routes may reuse **`discogs_user_id`** / **`discogs_username`** session cookies set at login so pagination stays one Discogs call per page; cookie-derived identity is **not** written into the OAuth token cache. OAuth tokens are still required and Discogs validates them on the data request. Write routes and `/api/auth/check` keep the default fresh verification policy and never authorize from display cookies alone.

**Release detail batching:** **`POST /api/release/batch`** accepts `{ ids: string[] }` (max **`RELEASE_BATCH_MAX_IDS`**, default 50), fetches each release from Discogs through the shared throttle, and returns `{ releases: Record<id, detail> }` (invalid or failed ids are omitted). The similar-release playback queue prefetches uncached ids in one batch via **`api.discogsReleaseBatch`** ([`similarReleaseQueue.ts`](../../src/utils/similarReleaseQueue.ts)).
...
**App DB rate limits:** Verified-user routes call [`getVerifiedUserFromRequestWithRateLimit`](../../src/lib/api-helpers.ts) with separate read/write buckets ([`rate-limit.ts`](../../src/lib/rate-limit.ts) — **`DB_RATE_LIMIT_*`**, default **60** writes/min). Crate layout saves and gig-packing toggles count as writes; **`useSetReleasePackedInCrateMutation`** and **`useUpdateCrateLayoutMutation`** retry transient **429** using **`Retry-After`** when the API returns [`ApiFetchError`](../../src/api/apiFetchError.ts).

**Discogs request pacing:** [`discogs-oauth.service.ts`](../../src/services/discogs-oauth.service.ts) serializes outbound Discogs HTTP through [`discogs-request-throttle.ts`](../../src/lib/discogs-request-throttle.ts). The throttle is **reentrancy-safe** (nested calls inside an active slot run immediately via **`AsyncLocalStorage`**) and rejects waiters after **`DISCOGS_THROTTLE_QUEUE_TIMEOUT_MS`** (default **30s**) so a stuck slot cannot pin an invocation for the full Fluid timeout — **`GET /api/release/[id]`** maps that **`DiscogsThrottleQueueError`** to **503** with **`Retry-After: 5`**, and **`useDiscogsReleaseQuery`** retries transient **503** / **429** like collection pagination. Concurrent **GET**s for the same Discogs URL on one serverless instance share one in-flight OAuth request (deduped in **`discogs-oauth.service.ts`**). Prefer a single throttle boundary — do not wrap **`makeAuthenticatedRequest`** / **`makeConsumerRequest`** in another **`runThrottledDiscogsRequest`**. **`getIdentity`** (OAuth callback / session verify) uses **`skipThrottle: true`** so login never waits behind collection pacing. Every Discogs HTTP call goes through [`discogsFetch`](../../src/lib/discogs-fetch.ts) with **`AbortSignal.timeout`** (**`DISCOGS_FETCH_TIMEOUT_MS`**, default **20s**). After each Discogs response, [`discogs-rate-limit.ts`](../../src/lib/discogs-rate-limit.ts) reads **`X-Discogs-Ratelimit`**, **`X-Discogs-Ratelimit-Used`**, and **`X-Discogs-Ratelimit-Remaining`** and adjusts spacing before the next call on that serverless instance (faster when quota remains, slower when the window is nearly exhausted). When Discogs omits those headers, pacing falls back to **`DISCOGS_MIN_REQUEST_INTERVAL_MS`** (default **1000** ms; set **0** to disable the fallback floor).
...
**IndexedDB collection cache:** after the collection is fully loaded, [`useCollectionData`](../../src/hooks/useCollectionData.hook.ts) persists React Query pages to IndexedDB via [`collectionCacheStorage.ts`](../../src/utils/collectionCacheStorage.ts) / [`collectionCacheSync.ts`](../../src/utils/collectionCacheSync.ts), including after a validated cache hit when fresh pages arrive. Personal rating and notes writes also patch the persisted cache immediately via [`patchPersistedCollectionReleaseRating`](../../src/utils/collectionCacheSync.ts) / [`patchPersistedCollectionReleaseNotes`](../../src/utils/collectionCacheSync.ts). [`CollectionCachePrepGate`](../../src/components/CollectionCachePrepGate/CollectionCachePrepGate.component.tsx) calls [`useCollectionCacheReady`](../../src/hooks/useCollectionCacheReady.hook.ts) as soon as auth exposes a username ([`resolveCollectionCacheUsername`](../../src/utils/resolveCollectionCacheUsername.ts) — active session or soft-logout **`reconnectUsername`**) so the login page can warm IndexedDB before OAuth completes. [`prepareCollectionQueryFromCache`](../../src/utils/collectionCacheSync.ts) validates with a single page-1 request comparing **`pagination.items`** **before** hydrating the infinite query from disk; when validation passes, cached pages load instantly, and when it fails (collection size changed on Discogs), IndexedDB is cleared and a full re-fetch runs on the same visit—stale pages are never injected into React Query. After OAuth success, **`clearUserScopedQueries`** clears in-memory collection data but a subsequent prepare call re-hydrates from IndexedDB when still valid. [`useCollectionData`](../../src/hooks/useCollectionData.hook.ts) syncs hydrated pages into **`allReleasesAtom`** with **`useLayoutEffect`** (before paint). Flow: (1) IndexedDB prep validates and may hydrate React Query; (2) once prep marks **`ready`**, the collection infinite query enables — if pages are already in cache, **`refetchOnMount: false`** keeps the grid instant on refresh; otherwise Discogs pagination runs. Validation failure or timeout (**`COLLECTION_CACHE_VALIDATION_TIMEOUT_MS`**, 10s) skips hydration and the live fetch runs after prep settles. [`useCollectionCacheReady`](../../src/hooks/useCollectionCacheReady.hook.ts) shares one ready state per username; prep always marks **`ready: true`** when its promise settles. [`useCollectionLoadState`](../../src/hooks/useCollectionData.hook.ts) reports loading while prep is in flight, the first page is fetching, or pagination continues. IndexedDB entries are kept indefinitely until that count check fails or the user runs **Clear All Data**. When validation passes after a cache hit, [`useCollectionData`](../../src/hooks/useCollectionData.hook.ts) runs an automatic **Sync collection** pass (same crate cleanup as Settings → Data) without a confirmation dialog; removals toast, zero removals stay silent. [`useDiscogsCollectionQuery`](../../src/hooks/queries/useDiscogsCollectionQuery.ts) uses **`COLLECTION_CACHE_STALE_MS` (24 hours)** for in-memory **`staleTime`** / **`gcTime`**. **Clear All Data** wipes the IndexedDB store via [`clearClientStoredData`](../../src/utils/clearClientStoredData.ts).
...
**Card click:** On **`/releases`**, clicking **cover art** or the overlay **Release details** button on desktop/mobile cards opens the release detail modal via **`onReleaseClick`**. On **`/crate/[id]`** (public crates), clicking **cover art** opens **`PublicReleaseModal`** (tracklist + playback via **`useRegisterPlaybackReleaseClick`**) — no collection notes, no crate toolbar, and format/style pills are static (not filter actions). **Title**, artist, and label text on cards and in modal heroes link to Discogs in a new tab (**`ReleaseHeaderLinks`** + **`ReleaseCardMeta`**; heroes merge ids from **`useDiscogsReleaseQuery`** via [`releaseHeaderMetadata.ts`](../../src/utils/releaseHeaderMetadata.ts) when collection **`resource_url`** is missing). The public modal toolbar includes **View on Discogs** and **Close** only.
...
Display/edit UI lives in [`src/components/ReleaseNotes/`](../../src/components/ReleaseNotes/). Note labels, search text, write helpers, and **multi text-field picker** helpers (**`sortTextCollectionFields`**, **`getInitialActiveTextFieldId`**, **`getReleaseNotesTextFieldPickerOptions`**, **`parseReleaseNotesTextFieldPickerValue`**, **`RELEASE_NOTES_TEXT_FIELD_PICKER_LABEL`**) are in [`src/utils/releaseNotes.ts`](../../src/utils/releaseNotes.ts). Picker helpers expect text fields **already sorted** by **`sortTextCollectionFields`** (Discogs **`position`**).
...
**Editing scope:** text and textarea fields (**`isEditableCollectionField`**) plus Media/Sleeve Condition dropdowns (**`isConditionCollectionField`**, **`getEditableConditionFields`**) via shared **`ReleaseNotesFormFields`**. When the user has **more than one** text/textarea field, a **Note Field** **`Select`** shows one active textarea; option labels include link-colored **`(N chars)`** when trimmed value is non-empty (**`Select`** **`storedCharCount`**). All note field labels compose **[`field-label.module.css`](../../src/styles/modules/field-label.module.css)** (same uppercase muted style as filter controls and condition **`Select`** labels). Card editing uses **`NoteEditDialog`**; **`ReleaseModal`** uses **`ReleaseNotesModalEditor`** inline; crate rows use **`ReleaseNotesCrateScratchpad`** with the same picker when multiple text fields exist. **`releaseHasStoredConditionNotes`** keeps the modal notes section visible when only condition values are set. Condition values stay hidden from release-card/table display (**`forCard: true`** / **`isCardDisplayNoteField`**).
...
**List UI:** Desktop **`ReleasesTable`** exposes notes only via the sticky-note icon in the trailing actions column (**`ReleaseNotesCardAction`** **`variant="table"`** inside **`ReleaseNotesEditorProvider`**). Other list contexts may use **`ReleaseNotes`** **`inline`** (note text plus **Add/Edit release notes** without the card provider).
