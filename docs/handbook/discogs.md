...
**Ratings:** Collection pagination includes your personal **`rating`** (0–5) per instance. Release detail adds **`community.rating`** for the Discogs-wide average. **`ReleaseSummaryHero`** appends your rating and the community average to the label/year meta line when available (community average uses a star icon instead of the word “Community”). Format and style filter pills remain below that meta line. Sort options include **Your Rating** (collection field) and **Community Rating** (Discogs average fetched via **`GET /api/release/{id}/rating`**, cached in **`localStorage`** through [`communityRatingsStorage.ts`](../../src/utils/communityRatingsStorage.ts)). When community sort is active, [`useCommunityRatingsLoader`](../../src/hooks/useCommunityRatingsLoader.hook.ts) loads ratings for the **filtered** collection only, one request about every 1.1s, and pauses for 60s after a Discogs **429** so release-detail fetches (tracklist) are not starved.
