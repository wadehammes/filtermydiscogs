...
| `PersistentYoutubeIframe.component.tsx` | YouTube embed iframe (hidden or visible). Registers with **`ReleasePlaybackProvider`**, calls **`notifyPlaybackIframeLoaded`** on **`load`** so [`youtubeIframeEvents.ts`](../../src/utils/youtubeIframeEvents.ts) can subscribe to ended events for queue auto-advance |
