...
| `ReleaseTracklist.component.tsx` | Track rows are clickable only when **`isTrackPlayable`** returns true (matched embeddable video); otherwise static text rows with muted titles and no hover. Optional **`hideTrackPosition`** drops the position column (preview videos). **`reserveQueueColumn`** keeps an empty queue slot on non-playable rows so durations align with playable rows and preview videos. Per-track **`artists`** / **`extraartists`** credits on Various Artists comps; visible **Add to queue** (list-plus icon) on playable rows; animated bars or pause icon on the dock’s active track |
