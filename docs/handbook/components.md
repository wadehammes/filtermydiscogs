...
| Overlay actions | **`ReleaseCardOverlayActions`**: desktop cover overlay uses a segmented group (release details **`VinylThinIcon`**, **Add to queue** **`ListPlusThinIcon`** / **`CheckThinIcon`** when every playable track is already queued — or a **`Spinner`** while release detail is fetched — via **`useReleaseCardQueueAction`** (adds all embeddable album tracks not already queued, same rules as the modal **Add all to queue** toolbar), notes, View on Discogs) plus a **standalone crate** button to its right; mobile keeps a single full-height column (details → queue → notes → Discogs → crate last) with **`border-top`** separators |
...
| File | Role |
|------|------|
| [`GlobalPlaybackDock.component.tsx`](../../src/components/GlobalPlaybackDock/GlobalPlaybackDock.component.tsx) | Fixed viewport dock; sets **`data-global-playback-dock`** when **`isMiniPlayerVisible`** and loads [`playback-dock.module.css`](../../src/styles/modules/playback-dock.module.css) (`:global` layout tokens on **`html:has([data-global-playback-dock])`**, spacers, scroll padding). Renders **[`ReleaseMiniPlayer`](../../src/components/ReleaseMiniPlayer/ReleaseMiniPlayer.component.tsx)**. |
...
| [`BottomDrawer.component.tsx`](../../src/components/BottomDrawer/BottomDrawer.component.tsx) | Mobile bottom sheet (crate drawer, filters drawer, mobile menu): portaled to **`document.body`**, **`position: fixed`**, slide-up animation. **`usePlaybackPageScrollLock(isOpen)`** while open unless **`hideOverlay`** or **`inline`**. **`max-height`** uses **`svh`** and clears **`--sticky-header-total-height`** ( **`aboveMiniPlayer`** / **`behindMiniPlayer`** adjust for the mini-player dock — see module CSS). Optional **`dataAttribute`** on the overlay, or on the drawer when **`hideOverlay`** / **`inline`**, for global layout hooks (e.g. **`data-filters-drawer-open`**, **`data-mobile-menu-open`**, **`data-playback-queue-open`**). Scroll inside **`.content`** only. |
| `ReleaseMiniPlayer.component.tsx` | Fixed bottom dock via **`miniPlayerRoot`** (cover, title, prev/play-pause/next). Visible while **`isMiniPlayerVisible`** (active release session), including when paused. **Queue** button opens **[`PlaybackQueueDrawer`](../../src/components/PlaybackQueueDrawer/PlaybackQueueDrawer.component.tsx)** with a count badge. Video panel above the bar on mobile; on desktop, a floating panel defaults above the release metadata (left of the transport controls). Other drawers use **[`BottomDrawer`](../../src/components/BottomDrawer/BottomDrawer.component.tsx)** **`aboveMiniPlayer`** above the dock via inherited CSS vars from **`html:has([data-global-playback-dock])`**. Cover/title calls the registered **`onReleaseClick`** handler when the active page provides one. |
...
| [`SettingsClient`](../../src/components/Settings/SettingsClient.component.tsx) | `/settings` | Sidebar navigation + section panels (account, appearance, playback, filters, collection, data); shared [`SettingsBooleanPreferenceToggle`](../../src/components/Settings/SettingsBooleanPreferenceToggle.component.tsx) for checkbox prefs |
