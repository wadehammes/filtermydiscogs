...
| `ReleaseSummaryHero.component.tsx` | Cover + metadata (label, year, catno, format/style pills) + crate add/remove toggle + Discogs link + optional close button (modal header) |
...
| File | Role |
|------|------|
| `ReleaseMiniPlayer.component.tsx` | Video panel above the video toggle in the transport cluster; bar has cover/title, transport, crate +/-. On mobile the release meta row is centered above a wider-spaced transport row (`2.5rem` controls, `var(--space-2)` gap). [`ReleasePlaybackVideoPanel`](../../src/components/ReleasePlayback/ReleasePlaybackVideoPanel.component.tsx) is full viewport width on mobile; bar height tokens on `.withMiniPlayer` offset page padding, the mobile crate FAB, and Back to top. |
