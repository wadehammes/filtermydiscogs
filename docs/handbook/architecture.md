...
[`Providers.tsx`](../../src/components/Providers.tsx) wires **QueryClient**, **JotaiProvider**, theme, auth, collection, filters, and view providers for every route. Heavy authenticated-only providers (**`ReleasePlaybackProvider`**, **`CrateProvider`**, **`CollectionDataSync`**, **`GlobalPlaybackDock`**) load through [`AuthenticatedProvidersGate`](../../src/components/AuthenticatedProvidersGate.component.tsx) → dynamically imported [`AuthenticatedProviders`](../../src/components/AuthenticatedProviders.tsx) when the user is signed in or auth is still checking on a protected route ([`protectedRoutes.ts`](../../src/constants/protectedRoutes.ts)). **`AppToaster`** (Sonner) sits under `ThemeProvider`; **`AuthCheckingToast`** and **`LogoutOverlay`** render inside the view subtree. See [patterns.md → Provider stack](patterns.md#provider-stack).
