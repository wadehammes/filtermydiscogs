...
- **UI libraries**: **Base UI** (`@base-ui/react`) for headless dialogs/popovers/toasts (styled via **`AppDialog`**, **`AppToaster`**, and CSS Modules), **TanStack Table** (releases list), **TanStack Charts** (dashboard), **Sharp** (image proxy for mosaic).
...
- **Pages**: [`page.tsx`](../../src/app/page.tsx) (login/home), [`releases/`](../../src/app/releases/), [`crates/`](../../src/app/crates/) (hub + [`[id]/`](../../src/app/crates/[id]/page.tsx) owner workspace), [`dashboard/`](../../src/app/dashboard/), [`mosaic/`](../../src/app/mosaic/), [`settings/`](../../src/app/settings/), [`crate/[id]/`](../../src/app/crate/[id]/) (public share), [`about/`](../../src/app/about/), [`legal/`](../../src/app/legal/), [`admin/`](../../src/app/admin/).
- **API**: Discogs proxy (`collection`, `collection/fields`, `collection/value`, `collection/instances/...`, `search`, `release/[id]`, **`release/batch`**), auth (`auth/discogs`, `auth/callback`, `auth/check`, `auth/logout`, `auth/clear-data`), crate CRUD under `crates/` (including **`crates/migrate`**), `user/preferences`, `tracks/record`, `tracks/stats`, `build-version`, `image-proxy`, `dashboard/most-crated`, `dashboard/top-tracks`, `usage/events`, `cron/product-analytics`, admin stats + admin user lookup, dynamic OG (`og/crate/[id]`).
- **Root layout**: [`layout.tsx`](../../src/app/layout.tsx) — fonts, metadata defaults, `Providers`, Google Tag Manager.
- **Root error UI**: [`error.tsx`](../../src/app/error.tsx) (under Providers) and segment **`error.tsx`** files share [`ErrorFallbackView`](../../src/app/ErrorFallbackView.tsx) + [`error-boundary.module.css`](../../src/styles/modules/error-boundary.module.css) (centered card, no **`Page`**). [`global-error.tsx`](../../src/app/global-error.tsx) replaces the root layout — own `html`/`body`, no Providers, no **`src/components`** imports (uses **`ErrorFallbackView`** + the same CSS module only). Dev-only details via [`isLocalDevHost()`](../../src/utils/isLocalDevHost.ts). Theme: **`useLayoutEffect`** + [`applyThemeFromStorage()`](../../src/utils/applyThemeFromStorage.ts) on **`global-error`** (uses [`resolveThemeInitAttribute`](../../src/utils/themeAppearance.ts), same rules as generated [`public/theme-init.js`](../../public/theme-init.js); no **`<script>`** — client components cannot execute head scripts in React 19).
...
**Authenticated shells**: [`ReleasesClient`](../../src/components/ReleasesClient/ReleasesClient.component.tsx) (primary collection UI), [`DashboardClient`](../../src/components/Dashboard/DashboardClient.component.tsx), [`MosaicClient`](../../src/components/MosaicClient/MosaicClient.component.tsx), [`SettingsClient`](../../src/components/Settings/SettingsClient.component.tsx), [`CratesClient`](../../src/components/Crates/CratesClient.component.tsx) / [`CrateDetailClient`](../../src/components/Crates/CrateDetailClient.component.tsx), [`AdminDashboardClient`](../../src/components/AdminDashboard/AdminDashboardClient.component.tsx).

[`Providers.tsx`](../../src/components/Providers.tsx) wires **QueryClient**, **JotaiProvider**, theme, auth, collection, filters, and view providers for every route. **`PlaybackReleaseClickProvider`**, **`CrateProvider`**, **`CollectionCachePrepGate`** (IndexedDB → React Query as soon as a username or soft-logout **`reconnectUsername`** is known), and **`AuthenticatedCollectionSyncGate`** (`CollectionDataSync` when signed in) are always mounted. Heavy authenticated-only providers (**`ReleasePlaybackProvider`**, **`GlobalPlaybackDock`**) load through [`AuthenticatedProvidersGate`](../../src/components/AuthenticatedProvidersGate.component.tsx) → dynamically imported [`AuthenticatedProviders`](../../src/components/AuthenticatedProviders.tsx) when the user is signed in or auth is still checking on a protected route ([`protectedRoutes.ts`](../../src/constants/protectedRoutes.ts)). **`AppToaster`** (Base UI Toast) sits under `ThemeProvider`; session toasts and **`LogoutOverlay`** render inside the gate wrapper. See [patterns.md → Provider stack](patterns.md#provider-stack).
