...
- **API**: Discogs proxy (`collection`, `collection/fields`, `collection/value`, `collection/instances/...`, `search`, `release/[id]`, **`release/batch`**), auth (`auth/discogs`, `auth/callback`, `auth/check`, `auth/logout`, `auth/clear-data`), crate CRUD under `crates/` (including **`crates/migrate`**), `image-proxy`, `dashboard/most-crated`, `usage/events`, `cron/product-analytics`, admin stats + admin user lookup, dynamic OG (`og/crate/[id]`).
...
- **[`.github/workflows/ci.yml`](../../.github/workflows/ci.yml)** — PR checks into `staging`: single **Lint/Test** job (`tsc:ci` includes `contract:emit`, `lint:ci`, `lint:css`, `test:ci`, `test:e2e:ci`, `knip:ci`).
