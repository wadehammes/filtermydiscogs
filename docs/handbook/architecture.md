...
- **UI libraries**: **Base UI** (`@base-ui/react`) for headless dialogs/popovers (styled via **`AppDialog`** and CSS Modules), **TanStack Table** (releases list), **TanStack Charts** (dashboard), **Sonner** (toasts), **Sharp** (image proxy for mosaic).
...
- **API**: Discogs proxy (`collection`, `collection/fields`, `collection/value`, `collection/instances/...`, `search`, `release/[id]`), auth (`auth/discogs`, `auth/callback`, `auth/check`, `auth/logout`, `auth/clear-data`), crate CRUD under `crates/`, `image-proxy`, `track-metadata` (GetSongBPM BPM/key batch lookup), `dashboard/most-crated`, `usage/events`, `cron/product-analytics`, admin stats + admin user lookup, dynamic OG (`og/crate/[id]`).
