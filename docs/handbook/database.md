...
- **ORM**: **Prisma 8 RC** — keep **`@prisma/orm-postgres`**, **`prisma`**, and **`@prisma/cli-engine`** aligned in [`package.json`](../../package.json). Current pins: **`@prisma/orm-postgres@8.0.0-rc.11`** (runtime; latest on npm for the ORM package), **`prisma@8.0.0-rc.15`** (CLI), **`@prisma/cli-engine@0.4.0`** (matches the rc.15 CLI). **`@prisma/orm-postgres` must be rc.11+** so PSL emit writes **`nullable`** on every to-one relation; an older ORM with a newer CLI fails **`pnpm contract:emit`** with **`CONTRACT.RELATION_INVALID`** even when [`contract.prisma`](../../src/prisma/contract.prisma) is unchanged. Do not float **`prisma`** ahead with **`^`** while the ORM pin stays on an older RC. Validate on staging/preview only until **`8.0.0` GA**; do not deploy RC builds to production.
- **Database**: PostgreSQL (Vercel Postgres in production)
- **Client**: [`src/lib/db.ts`](../../src/lib/db.ts) — contract runtime (`db`, `orm`, `countRows`, `queryRawRows`, `ormTimestamp`, `ormDate`, `sqlDate`, `sqlTimestamp`, `toOrmDate`, `toOrmJson`) backed by a shared `pg` pool. **`db`** and **`orm`** are lazy Proxies — importing the module does **not** require **`DATABASE_URL`** (public pages / Playwright e2e); the pool connects on first property access. Normalizes **`sslmode=require`** / **`prefer`** / **`verify-ca`** to **`verify-full`** (current `node-pg` semantics; silences the upcoming pg v9 alias warning) and enforces **`verify-full`** in production when SSL mode is omitted.

**Contract** (source of truth): [`src/prisma/contract.prisma`](../../src/prisma/contract.prisma). Emitted artifacts: [`src/prisma/contract.json`](../../src/prisma/contract.json), [`src/prisma/contract.d.ts`](../../src/prisma/contract.d.ts). CLI config: [`prisma.config.ts`](../../prisma.config.ts). After **`pnpm contract:emit`** on the rc.10+ toolchain, **`contract.d.ts`** exports a **`Models`** namespace (and related **`RelationKeys`** wiring) for typed row shapes — regenerate and commit both emitted files when upgrading Prisma; do not edit them by hand.
...
Query pattern (fluent chaining): `orm.Crates.where((crate) => crate.userId.eq(userId)).orderBy(...).all()`. Per-crate release totals for list UIs: [`fetchCrateReleaseCountsByCrateId`](../../src/lib/crate-release-counts.server.ts) (`CrateReleases` **`groupBy("crateId")`** + **`aggregate.count()`**). Pagination: **`.offset(n).limit(m)`** (not `.skip`/`.take`). Row counts: `countRows(orm.Users.where(...))`. Transactions: `db.transaction(async (tx) => tx.orm.public.…)`.
...
|-------|---------|---------|
| `/api/crates` | GET, POST | List/create crates for authenticated user; GET returns crates ordered **name ascending** (`releaseCount` and up to three `previewThumbs` per crate). Pass **`?all=true`** to return up to **`CRATE_LIST_ALL_MAX`** (500) crates in one page (used by the client crate list) |
...
| `/api/crates/migrate` | POST | Bulk import legacy localStorage releases into the user's default crate (`{ releases }`, max **`LEGACY_CRATE_MIGRATE_MAX`**); resolves the target crate via **`isDefault: true`**, then the first crate by name when none is marked default |
