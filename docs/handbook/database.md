...
| `extendQueueWithSimilarReleases` | `false` | When `true`, playback appends one related collection track at a time when up next runs low |
| `theme` | `"system"` | App color theme (`amber`, `codex`, `dark`, `dim`, `discogs`, `forest`, `futuristic`, `high-contrast`, `light`, `midnight`, `sepia`, `slate`, `wine`, or `system`) |
...
Owner-only section labels within a crate layout (e.g. “Peak hour”, nested “Deep House” under “Early Set”). Composite primary key: **`[user_id, crate_id, id]`** (UUID). **`sort_order`** interleaves with releases in the unified layout. Optional **`parent_id`** (self-FK, max depth 2) for nested groups; optional **`accent_key`** (curated palette → `--crate-section-accent-*` in [`base.css`](../../src/styles/themes/base.css)). Not returned on public crate routes.

**`CrateRelease.section_id`** (nullable FK → marker) stores explicit group membership for layout saves. On the owner crate page, only releases with **`section_id`** set count as section members (styled inside the group); other rows in the same structural block keep a neutral background. Dragging directly under a section header (or below another member) assigns membership; inserting **above** the first unsectioned release below a member run does **not** join the group. Dragging out clears it. On save, stale **`section_id`** values are cleared when the marker is missing, the release sits above its marker, or an unsectioned row appears between earlier members and the release (legacy “everything below the marker” data). **`CrateLayoutList`** wraps each marker’s structural block in a **`sectionGroup`** element (header [`CrateSetMarkerRow`](../../src/components/Crates/CrateSetMarkerRow.component.tsx) plus **`.sectionBody`**). When a group has no member releases, the body sets **`data-section-empty`** and reuses the **Add section** insert row (**`section-empty:{markerId}`** droppable id via **`getCrateLayoutSectionEmptyDropId`**) so clicking **+** or dropping a release both target the slot under the header; the portaled line indicator stays off over that zone (only above/below sortable rows elsewhere). When the body has members, a dashed **Drop release here** footer (**`section-end:{markerId}`** via **`getCrateLayoutSectionMemberEndDropId`**) mounts an **absolute** bottom hit target while dragging a release or section marker (dashed accent **`color-mix`** chrome only on **`isOver`**) and inserts after the last member (before unsectioned rows in the same structural block). Nested subsections that are the last body segment set **`suppressMemberEndDrop`** so only the parent footer shows; **`shouldShowCrateLayoutInsertZoneAtSectionBodyTail`** adds an **Add section** row below the nested card when needed. Nested **`.sectionGroupNested`** cards inside **`.sectionBody`** use inset **`border-radius`**, border, background, and **`overflow: hidden`**. **`moveCrateLayoutSectionBeforeSortableId`** accepts **`section-end:*`** when moving a descendant subgroup onto the parent footer. No duplicate insert row renders below an empty group. When **`accent_key`** is set, the group **`li`** also gets **`data-section-accent`** and defines **`--crate-section-inner-border*`** tokens so inner chrome (header rule, row dividers, notes inputs, marker label/add-group controls, section and member drag handles, empty drop, outer border) **`color-mix`**es the palette accent instead of plain **`--border`**. Section-head corner radius in [`CrateLayoutList.module.css`](../../src/components/Crates/CrateLayoutList.module.css) keys off **`data-section-empty`** (**`.sectionGroup:has(> .sectionBody[data-section-empty])`**) rather than marker-only **`:has()`** in [`CrateSetMarkerRow.module.css`](../../src/components/Crates/CrateSetMarkerRow.module.css). When more layout rows follow a group, **`buildCrateLayoutRenderSegments`** sets **`showInsertAfter`** / **`insertAfterIndex`** on section segments so the **Add section** split control renders **below** the group border (including before unsectioned releases that still sit in the structural block). In-section member rows only show inline splits before another member or nested subsection, not before rows rendered outside the chrome. Existing markers backfill with **`parent_id` null**; the section-tree migration backfilled **`section_id`** once from layout order.

**Layout lib (owner DnD):** [`crate-layout.ts`](../../src/lib/crate-layout.ts) builds the merged release/marker list, filters **visible** rows when **hide packed** is on (`getVisibleCrateLayoutItems`, `countVisibleCrateReleases`), merges a **visible-only reorder** back into the full list without moving hidden packed rows (`mergeReorderedVisibleCrateLayout`), normalizes sort orders for PUT payloads, and reorders with **`reorderCrateLayoutItems`** (insert before/after the **`over`** row from pointer **Y** vs the row midpoint, matching the portaled line). List-level **Add section** slots expose release droppable ids **`layout-insert:{visibleIndex}`** via **`getCrateLayoutListInsertDropId`** / **`getCrateLayoutListInsertIndexFromDropId`**; **`reorderCrateLayoutReleaseToVisibleInsertIndex`** moves the dragged release to that visible index. On **`CrateDetailClient`**, toolbar **+** mounts are portaled outside the **`<ul>`**; **`layout-insert:0`** registers on the top mount only while dragging a release that is not already first (portaled line shows the insert edge—no dashed head zone in the list). The bottom **`layout-insert:{n}`** droppable still renders as a zero-height tail overlay ([`CrateLayoutListHeadInsert.module.css`](../../src/components/Crates/CrateLayoutListHeadInsert.module.css)) with a dashed hit target when dragging a non-last release. **`CrateLayoutList`** uses **`MeasuringStrategy.Always`**, **`over ?? collisions[0]`** on **`onDragOver`**, and a portaled **`.layoutDropIndicator`**; sibling rows keep **`transform: none`** during drag. **`topInsertMount`** / **`bottomInsertMount`** portal toolbar chrome outside the **`<ul>`**. [`crate-section-layout.ts`](../../src/lib/crate-section-layout.ts) renders section segments, drop indicators, and section-id resolution; **`buildCrateLayoutRenderSegments`** and **`shouldShowCrateLayoutInsertZoneAfterLooseInLayout`** place **Add section** splits (see **`showInsertAfter`** / **`insertAfterIndex`** on section segments above). It treats **`section-empty:*`** ids like marker ids when reordering visible items; **`section-end:*`** drops append after the section member run via **`reorderCrateLayoutReleaseOverSectionMemberEnd`**. **`resolveCrateLayoutDropIndicatorForDrag`** returns **`null`** over **`section-empty:*`** and **`section-end:*`** (those targets use inline dashed zones), draws on **`layout-insert:*`**, and elsewhere **`CrateLayoutList`** shows a portaled **primary** line (2px) aligned to the insert edge from **`resolveCrateLayoutDropIndicator`**. **`getCrateLayoutSectionBlock`** for nested markers (**`parent_id` set**) ends before sibling member releases in the parent body ( **`section_id`** not equal to the subsection and not a descendant marker id), so subgroup drags do not swallow parent-level members; **`moveCrateLayoutSectionBeforeSortableId`** re-infers **`parent_id`** on moved markers and accepts parent-member release targets when the active marker belongs to that parent block. **`filterCrateLayoutMarkerDragCollisions`** still blocks cross-group member targets for unrelated section drags. **`applyResolvedCrateLayoutSectionIds`** sets **`forcedSectionId`** on the moved release when the drop target is **`section-empty:*`** or **`section-end:*`** (position-only inference cannot distinguish that slot from inserting above the first unsectioned row below members); otherwise it **`inferCrateLayoutSectionIdForReleaseIndex`** for the moved row. **`applyCrateLayoutListDragReorder`** is the drag-end orchestrator (release reorders merge visible changes back into the full list; **section-marker** drags move the section block on the **full** layout so hidden packed rows stay with their group). It then runs **`applyResolvedCrateLayoutSectionIds`**. **`CrateLayoutList`** calls that helper on **`onDragEnd`**, updates local state, then debounces **`updateCrateLayout`** (~300ms) through **`CrateProvider`**. Owner **Delete section** opens **`ConfirmDialog`** for every marker (including unsaved **`temp-marker-*`** rows), then runs **`reparentCrateLayoutMarkersAfterDelete`**, then the same PUT path (**`crateLayoutItemsToPutRequest`** → **`PUT /api/crates/[id]/layout`**). Server parsing lives in [`crate-layout.server.ts`](../../src/lib/crate-layout.server.ts): **`buildCrateLayoutUpdate`** validates the payload and resolves **`section_id`**; **`applyCrateLayoutUpdate`** upserts kept markers and runs **`crateSetMarker.deleteMany`** for ids not in **`markerIdsToKeep`** (an empty keep list deletes every marker row for that crate). Deleting the last section group must accept a releases-only payload with **`markersToUpsert: []`** and **`markerIdsToKeep: []`**, clearing release **`section_id`** via **`resolveCrateLayoutSectionIds`**. Regression coverage: [`crate-layout.spec.ts`](../../src/lib/crate-layout.spec.ts), [`crate-section-layout.spec.ts`](../../src/lib/crate-section-layout.spec.ts) (including empty-section drops), [`crate-layout.edge-cases.spec.ts`](../../src/lib/crate-layout.edge-cases.spec.ts) (single flat **`describe("crateLayoutEdgeCases")`**, including **`layout-insert:0`** with no groups), [`crate-layout.server.spec.ts`](../../src/lib/crate-layout.server.spec.ts) (**`buildCrateLayoutUpdate`**, including group-delete save), and pointer DnD wiring in [`CrateLayoutList.spec.tsx`](../../src/components/Crates/CrateLayoutList.spec.tsx) (row reorder, edge-mount top insert drop, section-delete confirm).
...
- **`useCrateMigration`** handles legacy localStorage → server migration on login via **`useMigrateLegacyCrateMutation`** → **`POST /api/crates/migrate`** (one request, then invalidates crate queries).
