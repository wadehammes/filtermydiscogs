...
- **UI**: [`CrateDrawer`](../../src/components/CrateDrawer/CrateDrawer.component.tsx) sidebar on desktop (≥1024px) and bottom drawer on mobile — a **staging** surface on **`/releases`** only: [`CrateSelector`](../../src/components/CrateSelector/CrateSelector.component.tsx), a simple release list ([`CrateDrawerReleases`](../../src/components/CrateDrawer/CrateDrawerReleases.component.tsx) — cover/title/meta, **remove** only; no section markers, gig-packing toggles, or hide-packed filter), and footer **View/Edit Crate** (links to [`/crates/[id]`](../../src/app/crates/[id]/page.tsx)) plus **Empty**. On mobile, [`ReleasesClient`](../../src/components/ReleasesClient/ReleasesClient.component.tsx) wraps the drawer in **`.sidebar`** with **`display: contents`** so the bottom drawer is not suppressed by **`display: none`**. **`CrateDrawerProvider`** ([`CrateDrawer.context.tsx`](../../src/components/CrateDrawer/CrateDrawer.context.tsx)) wraps the drawer shell and calls **`useCrateDrawerState`** once; the same provider/context also powers owner pages (detail actions menu, set-notes scratchpad, packing toolbar). **Open/closed state** lives in [`useCrateDrawer`](../../src/hooks/useCrateDrawer.hook.ts): `isDrawerOpen = userToggled ?? isDesktop`. Use **`openDrawer()`** / **`closeDrawer()`** for explicit user actions; **`resetDrawer()`** clears `userToggled` back to viewport default. **`addToCrate`** opens the drawer only when **`isDesktop`**. After OAuth login, **`CrateProvider`** calls **`resetDrawer()`**; logout/user switch calls **`closeDrawer()`**. Mobile crate FAB sits above the mini player dock. Owner dialogs ([`EditCrateDialog`](../../src/components/EditCrateDialog/EditCrateDialog.component.tsx), [`CrateNotesDialog`](../../src/components/CrateDrawer/CrateNotesDialog.component.tsx), empty/clear-packed/make-default confirms) remain on **`CrateDrawerDialogs`** for the detail page; submit-style crate forms use **React Hook Form** ([conventions.md](conventions.md)).
- **Owner pages**: [`/crates`](../../src/app/crates/page.tsx) lists the user's crates in a responsive card grid ([`CrateHubCard`](../../src/components/Crates/CrateHubCard.component.tsx) — cover collage from the first three releases, name, and release count). [`/crates/[id]`](../../src/app/crates/[id]/page.tsx) is the authenticated owner workspace (`CrateDetailClient`): full-width layout with [`CrateDetailHeader`](../../src/components/Crates/CrateDetailHeader.component.tsx) (back link, [`CrateSelector`](../../src/components/CrateSelector/CrateSelector.component.tsx) **`onNavigate`** prefetch, [`CrateDetailActionsMenu`](../../src/components/Crates/CrateDetailActionsMenu.component.tsx) — **Edit crate**, **Set notes**, **Set as default**, **Make shareable**, **Copy link**, **Empty crate**), then a **two-column workspace** on desktop (≥1024px): release list on the left with a sticky [`CrateReleaseListToolbar`](../../src/components/Crates/CrateReleaseListToolbar.component.tsx) for gig-packing progress (**Clear packed**, **Hide packed**), and a sticky **set-notes scratchpad** on the right ([`CrateSetNotesScratchpad`](../../src/components/Crates/CrateSetNotesScratchpad.component.tsx) — debounced autosave, monospace input styling); stacked on mobile (set notes above the list). [`CrateLayoutList`](../../src/components/Crates/CrateLayoutList.component.tsx) drives the owner list: drag-and-drop reorder (**`@dnd-kit`**) for releases and **set markers** ([`CrateSetMarkerRow`](../../src/components/Crates/CrateSetMarkerRow.component.tsx) — inline label edit persists on blur, delete, **Add section**); per-release collection notes remain inline scratchpads ([`ReleaseNotes`](../../src/components/ReleaseNotes/ReleaseNotes.component.tsx) **`crate`** variant); desktop adds column headers (**Notes / Release / Actions**). Layout changes persist via **`PUT /api/crates/[id]/layout`** and **`useUpdateCrateLayoutMutation`**. **`CrateProvider`** exposes merged **`layoutItems`** (releases + markers by **`sort_order`**) alongside **`selectedReleases`**. **Hide packed** filters release rows in the UI but keeps the full layout (including hidden releases and marker positions) for saves. The [`/releases`](../../src/app/releases/page.tsx) drawer lists releases in the same **`sort_order`** as the owner layout (via **`layoutItems`**) but omits section markers ([`CrateDrawerReleases`](../../src/components/CrateDrawer/CrateDrawerReleases.component.tsx)). Create new crates from **`/releases`** drawer or the hub (`allowCreate={false}` on detail). Release rows open [`ReleaseModal`](../../src/components/ReleaseModal/ReleaseModal.component.tsx) inside **`ReleasePlaybackProvider`**. The URL crate id is the source of truth on the detail page (`selectCrate` from route param). Header nav includes **Crates** (`currentPage="crates"`). When **`packed_enabled`** is on, gig-packing UI (**Packed for gig** toggle, **Clear packed**, **Hide packed**) lives on the detail page only ([`CrateReleaseListToolbar`](../../src/components/Crates/CrateReleaseListToolbar.component.tsx), [`CrateLayoutList`](../../src/components/Crates/CrateLayoutList.component.tsx) rows).
...
**`/admin`** is gated by **`ADMIN_USER_ID`** env matching the `discogs_user_id` cookie. Stats come from **`/api/admin/stats`** via **`useAdminStatsQuery`**. User counts and growth charts use **`User.created_at`** (not first-crate heuristics).
