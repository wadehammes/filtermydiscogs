...
- **UI**: [`ReleaseMiniPlayer`](../../src/components/ReleasePlayback/ReleaseMiniPlayer.component.tsx) is a slim background-playback dock (cover, title, prev/play-pause/next/stop). By default the YouTube embed is hidden (audio-only). A **video** control in the transport cluster toggles a 16:9 embed in a panel **above that button**—keeping the bottom-left clear for **Back to top** (which only offsets for the bar, not the video panel). Cover/title opens **`ReleaseModal`**. Play/pause sends YouTube iframe commands via **`togglePlayback`**. [`PersistentYoutubeIframe`](../../src/components/ReleasePlayback/PersistentYoutubeIframe.component.tsx) keeps the embed mounted off-screen when collapsed. A track row click in **`ReleaseModal`** calls **`startPlayback`**; there is no in-modal video preview. Closing the modal keeps playback going. Prev/next advance within the current release tracklist (no auto-advance on video end in v1).
