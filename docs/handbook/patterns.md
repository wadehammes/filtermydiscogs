...
- **Sections**: Account (Discogs username, **Complete logout** via **`logout({ preserveTokens: false })`**), appearance (**`ThemeSwitcher variant="dropdown"`**, default view select), filters (**Remember filter selections** → **`preferences.persistFilters`**), collection sync (**`useCrateCollectionSync`**), stored data (**`useClearAllUserData`** with confirm dialog).
