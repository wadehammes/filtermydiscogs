...
- **UI**: [`ReleaseMiniPlayer`](../../src/components/ReleasePlayback/ReleaseMiniPlayer.component.tsx) is a slim background-playback dock (cover, title, prev/play-pause/next/stop). By default the YouTube embed is hidden (audio-only). A **video** control toggles a 16:9 panel ([`ReleasePlaybackVideoPanel`](../../src/components/ReleasePlayback/ReleasePlaybackVideoPanel.component.tsx)). On **mobile** the panel is full viewport width, docked above the mini-player bar, with a top close bar (no drag/resize). On **desktop** it defaults above the transport controls, right-aligned at full size as a floating panel — drag the top handle to reposition, drag the bottom-right handle to shrink (not enlarge), and double-click the top handle to reset layout; layout persists for the browser session until the viewport crosses back from mobile to desktop, when position and scale reset to that default. Native browser PiP is not used — YouTube iframe embeds do not expose reliable programmatic PiP from app code. Cover/title opens **`ReleaseModal`**. Play/pause sends YouTube iframe commands via **`togglePlayback`**. [`PersistentYoutubeIframe`](../../src/components/ReleasePlayback/PersistentYoutubeIframe.component.tsx) keeps the embed mounted off-screen when collapsed. A track row click in **`ReleaseModal`** calls **`startPlayback`**; there is no in-modal video preview. Closing the modal keeps playback going. Prev/next advance within the current release tracklist (no auto-advance on video end in v1).
