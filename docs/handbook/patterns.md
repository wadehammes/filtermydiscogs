...
- **Navigation**: Linked from the username dropdown ([`UserActions`](../../src/components/StickyHeaderBar/components/UserActions.tsx)) and mobile menu footer ([`MobileMenuDrawerFooter`](../../src/components/StickyHeaderBar/components/MobileMenuDrawerFooter.tsx)).
- **Sections**: Account (Discogs username, **Complete logout** via **`logout({ preserveTokens: false })`**), appearance (**`ThemeSwitcher variant="dropdown"`**), collection sync (**`useCrateCollectionSync`**), stored data (**`useClearAllUserData`** with confirm dialog).
