...
- **UI**: [`ReleaseMiniPlayer`](../../src/components/ReleasePlayback/ReleaseMiniPlayer.component.tsx) is a slim background-playback dock (cover, title, prev/play-pause/next/stop). By default the YouTube embed is hidden (audio-only). A **video** control toggles a 16:9 panel ([`ReleasePlaybackVideoPanel`](../../src/components/ReleasePlayback/ReleasePlaybackVideoPanel.component.tsx)). The dock video panel **auto-expands once** the first time playback becomes ready in the bar (tracked in **`localStorage`** via [`playbackVideoIntroStorage.ts`](../../src/utils/playbackVideoIntroStorage.ts)) so listeners discover the control; later sessions stay collapsed until toggled. On **mobile**, the panel also **auto-expands whenever autoplay playback starts** (e.g. a track row click in **`ReleaseModal`**) because mobile browsers block hidden/off-screen embed autoplay; listeners can still collapse it with the top close bar. On **mobile** the panel is full viewport width, docked above the mini-player bar, with a top close bar (no drag/resize). On **desktop** it defaults above the transport controls, right-aligned at full size as a floating panel — drag the top handle to reposition, drag the bottom-right handle to shrink (not enlarge), and double-click the top handle to reset layout; layout persists for the browser session until the viewport crosses back from mobile to desktop or a stored position no longer fits the viewport, when position and scale reset to that default. Native browser PiP is not used — YouTube iframe embeds do not expose reliable programmatic PiP from app code. Cover/title opens **`ReleaseModal`**. Play/pause sends YouTube iframe commands via **`togglePlayback`**. [`PersistentYoutubeIframe`](../../src/components/ReleasePlayback/PersistentYoutubeIframe.component.tsx) keeps a single embed mounted in the dock; when collapsed it stays off-screen for audio-only playback. A track row click in **`ReleaseModal`** calls **`startPlayback`**; closing the modal does not stop playback. **`startPlayback`** also schedules **`playVideo`** iframe commands (with short retries) so the originating click gesture can resume playback after release detail loads. Prev/next advance within the current release tracklist (no auto-advance on video end in v1).
