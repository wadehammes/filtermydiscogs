...
- **UI**: [`ReleaseMiniPlayer`](../../src/components/ReleasePlayback/ReleaseMiniPlayer.component.tsx) is a slim background-playback dock (cover, title, prev/play-pause/next/stop). By default the YouTube embed is hidden (audio-only). A chevron tab on the bar toggles a left-aligned embed in a panel **above the mini bar** (natural 16:9). Cover/title opens **`ReleaseModal`**. Play/pause sends YouTube iframe commands via **`togglePlayback`**. [`PersistentYoutubeIframe`](../../src/components/ReleasePlayback/PersistentYoutubeIframe.component.tsx) keeps the embed mounted off-screen when collapsed. A track row click in **`ReleaseModal`** calls **`startPlayback`**; there is no in-modal video preview. Closing the modal keeps playback going. Prev/next advance within the current release tracklist (no auto-advance on video end in v1).
