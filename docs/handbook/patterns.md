...
- **Sections**: Account (Discogs username, **Complete logout** via **`logout({ preserveTokens: false })`**), appearance (**`ThemeSwitcher variant="dropdown"`**, default view select), filters (**Remember filter selections** → **`preferences.persistFilters`**), collection sync (**`useCrateCollectionSync`**), stored data (**`useClearAllUserData`** with confirm dialog). Theme, default view, and filter-persistence changes show a **Preferences saved** Sonner toast (or **Failed to save preferences** on error) after **`PATCH`** via **`persistPreferences`** callbacks.
- **Account prefs**: **`User.preferences`** JSON stores **`theme`**, **`view`**, **`persistFilters`**, and **`filters`**. Types in [`userPreferences.types.ts`](../../src/types/userPreferences.types.ts) (`ViewMode` from [`view.types.ts`](../../src/types/view.types.ts)). All authenticated **`PATCH`** calls use [`usePersistUserPreferences`](../../src/hooks/usePersistUserPreferences.hook.ts) + [`userPreferencesPersistQueue.ts`](../../src/utils/userPreferencesPersistQueue.ts)—theme/view/filter UI, **Remember filter selections** in Settings, and first-login seed from [`useUserPreferencesSync`](../../src/hooks/useUserPreferencesSync.hook.ts). Do not call **`useUpdateUserPreferencesMutation`** directly in app code. Sync hydrates server prefs into theme context and Jotai atoms after login. Shared validation: [`view.types.ts`](../../src/types/view.types.ts), [`storedTheme.ts`](../../src/utils/storedTheme.ts), [`filters.types.ts`](../../src/types/filters.types.ts) + [`filtersStorage.ts`](../../src/utils/filtersStorage.ts), [`sortValues.ts`](../../src/constants/sortValues.ts); storage keys in [`storageKeys.ts`](../../src/constants/storageKeys.ts).
