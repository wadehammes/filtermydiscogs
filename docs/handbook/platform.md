...
| [`AuthenticatedProvidersGate`](../../src/components/AuthenticatedProvidersGate.component.tsx) | Inner gate with **`usePathname`** wrapped in **`<Suspense fallback={children}>`** so protected routes keep an instant shell while pathname resolves |
