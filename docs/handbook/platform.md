...
Wire-up (Cursor only): **`pnpm exec fallow agent install --harness cursor --without hooks --without skill`** — refreshes [`.cursor/mcp.json`](../../.cursor/mcp.json) after Fallow upgrades without recreating **`.agents/skills/fallow`** (Codex/Cursor stub) or commit hooks. Agent context comes from MCP tool descriptions plus the task map in **`AGENTS.md`** / **`CLAUDE.md`**. Optional local skill: read **`node_modules/fallow/skills/fallow/SKILL.md`** after **`pnpm install`**, or **`fallow agent install --user`** for **`~/.agents/skills/fallow`**.
...
| [`authenticated-dashboard.spec.ts`](../../e2e/authenticated-dashboard.spec.ts) | **`/dashboard`** hero, analytics, **On repeat** playback rows, and **In your crates** (MSW collection + dashboard APIs) |
...
| Glob rules | [`.cursor/rules/css-modules.mdc`](../../.cursor/rules/css-modules.mdc), [`api-routes.mdc`](../../.cursor/rules/api-routes.mdc), [`hook-specs.mdc`](../../.cursor/rules/hook-specs.mdc) |
| Project skills | [`.cursor/skills/README.md`](../../.cursor/skills/README.md) — routing, TS/React, tests, API, Prisma, React Query, filters, overlays, crates, playback, OAuth, stacked PRs, Fallow |
...
**`sessionStart`** injects the **`llms.md`** routing map; **`beforeSubmitPrompt`** ([`handbook-adherence-reminder.sh`](../../.cursor/hooks/handbook-adherence-reminder.sh)) repeats a short handbook checklist on each user message. **`preToolUse`** / **`postToolUse`** enforce handbook conventions (CSS, scaffold/factory placement, no comments/barrels). **`beforeShellExecution`** requires [`scripts/git-commit.sh`](../../scripts/git-commit.sh) for commits and blocks destructive git ([`block-destructive-git.sh`](../../.cursor/hooks/block-destructive-git.sh)). **Stop** hooks run targeted Jest checks, a **`prisma/schema.prisma`** → **`pnpm db:generate`** nudge ([`prisma-generate-nudge.sh`](../../.cursor/hooks/prisma-generate-nudge.sh)), or **`pnpm lint:all`** ([`lint-all-check.sh`](../../.cursor/hooks/lint-all-check.sh)) when the session changed meaningful source — one follow-up if checks fail. Hooks resolve **`pnpm`** through **mise** ([`run_pnpm`](../../.cursor/hooks/_lib.sh)). Requires `jq`, **mise** (or **`pnpm`** on `PATH`), and executable hook scripts.
