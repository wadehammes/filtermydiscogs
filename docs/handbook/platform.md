...
- **Security headers**: CSP (tighter in production), HSTS, frame options, etc. on `/`, `/api/*`, and static paths. Production CSP restricts **`connect-src`**, **`frame-src`**, and **`img-src`**; development keeps broader directives for local debugging. Playback embeds use **`youtube-nocookie.com`** — both **`*.youtube.com`** and **`*.youtube-nocookie.com`** must stay in **`frame-src`** / **`child-src`**. Landing **`LoginPreviewDemo`** (**react-player**) loads **`youtube.com/iframe_api`** on the page — keep **`*.youtube.com`** in **`script-src`** and **`connect-src`** (see [`next.config.ts`](../../next.config.ts)). Vercel preview **`script-src`** also allows **`vercel.live`** for the Live feedback widget.
